  ...
  <div class="header-toolbar" :class="{ 'header-toolbar--disabled': props.disabled }">
    ...
    <div class="header-toolbar__tools">
      <template v-for="(item, index) in visibleTools" :key="item.key">
        <!-- 添加行下拉面板 -->
        <el-dropdown
          v-if="item.tool === 'add-row'"
          :disabled="item.disabled || props.disabled"
          trigger="click"
          @command="handleAddRow"
        >
          <el-tooltip
            :content="item.tooltip || '添加行'"
        ...
        <!-- 行高调整下拉面板 -->
        <el-dropdown
          v-else-if="item.tool === 'row-height'"
          :disabled="item.disabled || props.disabled"
          trigger="click"
...
import type {
  DetailTableToolbarState,
  DetailTableToolbarItem,
  DetailTableRowHeightPreset
} from '@/types/detail-table'
...
const ROW_HEIGHT_PRESETS: {
  key: DetailTableRowHeightPreset
  label: string
  height: number
}[] = [
...
const ADD_ROW_OPTIONS: { key: string; label: string; count: number }[] = [
  { key: 'add-1', label: '添加1行', count: 1 },
  { key: 'add-5', label: '添加5行', count: 5 },
  { key: 'add-10', label: '添加10行', count: 10 }
]
...
const localState = ref<DetailTableToolbarState>({
...
const visibleTools = computed<DetailTableToolbarItem[]>(() => {
...
const addRowOptions = computed(() => ADD_ROW_OPTIONS)

// ============================================================
// 方法
// ============================================================

function getDefaultTools(): DetailTableToolbarItem[] {
  return [
    {
      key: 'add-row',
      tool: 'add-row',
...
function getIcon(item: DetailTableToolbarItem): string {
...
async function handleToolClick(item: DetailTableToolbarItem): Promise<void> {
  try {
    ...
    console.error('[DetailTable HeaderToolbar] tool click error:', err)
    ElMessage.error('操作执行失败')
  }
}

function handleAddRow(count: number): void {
  try {
    syncState()
    emit('change', 'add-row', { ...localState.value, count } as DetailTableToolbarState & {
      count: number
    })
  } catch (err) {
    console.error('[DetailTable HeaderToolbar] add row error:', err)
    ElMessage.error('添加行失败')
  }
}

function handleRowHeightChange(height: DetailTableRowHeightPreset): void {
  try {
    localState.value.rowHeight = height
    syncState()
    emit('change', 'row-height', { ...localState.value })
  } catch (err) {
    console.error('[DetailTable HeaderToolbar] row height change error:', err)
    ElMessage.error('行高设置失败')
  }
}

function getState(): DetailTableToolbarState {
