...
import type {
  RelatedInfoToolbarState,
  RelatedInfoToolbarItem,
  RelatedInfoRowHeightPreset
} from '@/types/relation-info'

const ROW_HEIGHT_PRESETS: {
  key: RelatedInfoRowHeightPreset
  label: string
  height: number
}[] = [
...
const localState = ref<RelatedInfoToolbarState>({
...
const visibleTools = computed<RelatedInfoToolbarItem[]>(() => {
...
function getDefaultTools(): RelatedInfoToolbarItem[] {
  return [
    {
      key: 'maximize',
      tool: 'maximize',
...
function getButtonType(item: RelatedInfoToolbarItem): string {
  if (item.tool === 'maximize' && localState.value.maximized) return 'primary'
  return 'default'
}

function getIcon(item: RelatedInfoToolbarItem): string {
...
async function handleToolClick(item: RelatedInfoToolbarItem): Promise<void> {
  try {
...
function handleRowHeightChange(height: RelatedInfoRowHeightPreset): void {
  try {
