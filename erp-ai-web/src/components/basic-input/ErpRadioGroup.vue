<template>
  <div
    class="basic-input basic-input--radio"
    :class="{
...
const props = withDefaults(defineProps<ErpRadioGroupProps>(), {
  disabled: false,
  loading: false,
  options: () => [],
  buttonStyle: false,
...
const emit = defineEmits<ErpRadioGroupEmits>()
...
function collectErrors(value: unknown, rules?: ValidatorRule[]): string[] {
  ...
  for (const rule of rules) {
    if (rule.required && (value === undefined || value === null || value === '')) {
      errors.push(rule.message || '请选择一个选项')
    }
  }
  return errors
}
...
defineExpose<ErpRadioGroupExpose>({
  validate,
  reset
})
</script>
