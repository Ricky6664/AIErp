<template>
  <div
    class="basic-input basic-input--datetime"
    :class="{
...
import type {
  ...
  ErpDateTimePickerEmits,
  ErpDateTimePickerExpose,
  ValidatorRule,
  ErpInputLinkageEvent
} from '@/types/basic-input'
import { useFormLinkage } from '@/composables/useFormLinkage'

const props = withDefaults(defineProps<ErpDateTimePickerProps>(), {
  disabled: false,
  loading: false,
  placeholder: '请选择日期时间',
...
const emit = defineEmits<ErpDateTimePickerEmits>()
...
defineExpose<ErpDateTimePickerExpose>({
  validate,
  reset
})
</script>
