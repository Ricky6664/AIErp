...
export function useDemoEditTableReadonly() {
  ...
  const isDisabled = computed(() => formMode.value === 'view')
  ...
  function toggleFormMode(): void {
    formMode.value = formMode.value === 'edit' ? 'view' : 'edit'
    ElMessage.info(
      formMode.value === 'view'
        ? '已切换到只读模式，所有可编辑单元格已禁用'
        : '已切换到编辑模式，所有可编辑单元格已启用'
    )
  }

  function handleViewDetail(id: number): void {
    router.push({ name: 'DemoEditTableReadonly', query: { ...route.query, detailId: id } })
  }
