const common = {
  confirm: 'Confirm',
  cancel: 'Cancel',
  save: 'Save',
  delete: 'Delete',
  edit: 'Edit',
  add: 'Add',
  search: 'Search',
  reset: 'Reset',
  submit: 'Submit',
  back: 'Back',
  close: 'Close',
  refresh: 'Refresh',
  export: 'Export',
  import: 'Import',
  yes: 'Yes',
  no: 'No',
  ok: 'OK',
  success: 'Success',
  error: 'Error',
  warning: 'Warning',
  info: 'Info',
  loading: 'Loading...',
  noData: 'No Data',
  total: 'Total {total} items',
  pleaseSelect: 'Please select',
  pleaseInput: 'Please input',
  // Login page
  login: {
    title: 'ERP AI Management System',
    desc: 'Intelligent Enterprise Resource Management',
    username: 'Username',
    password: 'Password',
    captcha: 'Captcha',
    rememberMe: 'Remember me',
    submit: 'Login',
    captchaTip: 'Click image to refresh captcha',
    loginSuccess: 'Login successful',
    loginFailed: 'Login failed',
    welcomeBack: 'Welcome back'
  },
  // Finance workbench
  finance: {
    workbench: {
      title: 'Finance Settings Workbench',
      desc: 'Overview of core finance settings metrics',
      kpiTitle: 'KPIs',
      currencyRateCount: 'Currency Rates',
      bankAccountCount: 'Bank Accounts',
      accountCount: 'Accounts',
      voucherWordCount: 'Voucher Words',
      chartTitle: 'Analytics',
      trendTitle: 'Monthly Trend',
      distTitle: 'Account Type Distribution'
    },
    voucherword: {
      title: 'Voucher Word Management',
      wordCode: 'Voucher Word Code',
      wordName: 'Voucher Word Name',
      sortOrder: 'Sort Order',
      status: 'Status',
      addTitle: 'Add Voucher Word',
      editTitle: 'Edit Voucher Word',
      confirmDelete: 'Confirm to delete this voucher word?'
    }
  },
  org: {
    workbench: {
      title: 'Organization Workbench',
      desc: 'Overview of core organization metrics',
      kpiTitle: 'KPIs',
      companyCount: 'Companies',
      departmentCount: 'Departments',
      positionCount: 'Positions',
      employeeCount: 'Active Employees',
      chartTitle: 'Analytics',
      deptTypeDist: 'Department Type Distribution',
      companyDeptCompare: 'Company Department Comparison',
      quickActions: 'Quick Actions',
      addCompany: 'Add Company',
      addDepartment: 'Add Department',
      addPosition: 'Add Position',
      orgChart: 'Organization Chart',
      loadFailed: 'Data loading failed',
      retry: 'Retry',
      recentRecords: 'Recent Records',
      recentCompany: 'Recent Companies',
      recentDept: 'Recent Departments',
      recentPosition: 'Recent Positions'
    },
    company: {
      addTitle: 'Add Company',
      editTitle: 'Edit Company',
      companyName: 'Company Name',
      companyShortName: 'Short Name',
      creditCode: 'Credit Code',
      creditCodePlaceholder: 'Enter 18-digit credit code',
      legalPerson: 'Legal Representative',
      registeredCapital: 'Registered Capital',
      address: 'Address',
      phone: 'Phone',
      phonePlaceholder: 'Enter phone (landline or mobile)',
      registeredCapitalUnit: '10K CNY',
      creditCodeInvalid: 'Please enter a valid credit code',
      creditCodeDuplicate: 'Credit code already exists',
      companyNameRequired: 'Company name is required',
      companyNameLength: 'Company name must be 2-100 characters'
    }
  },
  hrm: {
    workbench: {
      title: 'HRM Workbench',
      desc: 'Human Resource Management Overview',
      kpiTitle: 'Key Indicators',
      employeeCount: 'Active/Total Employees',
      newHires: 'New Hires This Month',
      openRecruitments: 'Open Recruitments',
      monthlySalary: 'Monthly Salary Total',
      departmentCount: 'Departments',
      chartTitle: 'Charts',
      employeeTrend: 'Employee Trend',
      deptDist: 'Department Distribution',
      attendanceTrend: 'Attendance Trend',
      recruitStatus: 'Recruitment Status',
      newEmployee: 'New Employees',
      attendanceRecord: 'Attendance Records',
      deptDistribution: 'Department Distribution',
      recruitStatusDistribution: 'Recruitment Status',
      loadError: 'Failed to load workbench data',
      noData: 'No Data'
    },
    employee: {
      total: 'Total Employees',
      active: 'Active Employees',
      newThisMonth: 'New Hires This Month',
      departments: 'Departments',
      name: 'Name',
      namePlaceholder: 'Enter name',
      department: 'Department',
      position: 'Position',
      status: 'Status',
      statusActive: 'Active',
      statusLeave: 'Resigned',
      statusProbation: 'Probation',
      employeeNo: 'Employee No.',
      employeeNoPlaceholder: 'Enter employee number',
      gender: 'Gender',
      genderMale: 'Male',
      genderFemale: 'Female',
      phone: 'Phone',
      phonePlaceholder: 'Enter phone number',
      email: 'Email',
      emailPlaceholder: 'Enter email',
      idCard: 'ID Card',
      idCardPlaceholder: 'Enter 18-digit ID number',
      positionPlaceholder: 'Enter position title',
      entryDate: 'Entry Date',
      detailTitle: 'Details',
      selectHint: 'Select an employee to view details',
      clickRowHint: 'Click a row to view employee details',
      tabBasic: 'Basic Info',
      tabArchive: 'Archive',
      tabAttendance: 'Attendance',
      tabSalary: 'Salary',
      archiveHint: 'Archive data pending integration',
      attendanceHint: 'Attendance data pending integration',
      salaryHint: 'Salary data pending integration',
      baseInfo: 'Basic Information',
      archiveInfo: 'Archive Information',
      archiveList: 'Archive List',
      addArchive: 'Add Archive',
      archiveEducation: 'Education',
      archiveMajor: 'Major',
      archiveSchool: 'Institution',
      archiveEmergencyContact: 'Emergency Contact',
      archiveBankCardNo: 'Bank Card No.',
      add: 'Add Employee',
      addTitle: 'Add Employee',
      editTitle: 'Edit Employee',
      recordCount: '{total} records'
    },
    archive: {
      totalArchives: 'Total Archives',
      enabled: 'Enabled',
      disabled: 'Disabled',
      newThisMonth: 'New This Month',
      employeeName: 'Employee',
      employeeNamePlaceholder: 'Search and select employee',
      education: 'Education',
      major: 'Major',
      majorPlaceholder: 'Enter major',
      school: 'Institution',
      schoolPlaceholder: 'Enter institution',
      emergencyContact: 'Emergency Contact',
      emergencyContactPlaceholder: 'Enter emergency contact',
      emergencyPhone: 'Emergency Phone',
      emergencyPhonePlaceholder: 'Enter phone number',
      address: 'Address',
      addressPlaceholder: 'Enter address',
      bankCardNumber: 'Bank Card No.',
      bankCardNumberPlaceholder: 'Enter bank card number',
      bankName: 'Bank Name',
      bankNamePlaceholder: 'Enter bank name',
      socialSecurityAccount: 'Social Security No.',
      socialSecurityAccountPlaceholder: 'Enter social security number',
      archiveDate: 'Archive Date',
      status: 'Status',
      statusEnabled: 'Enabled',
      statusDisabled: 'Disabled',
      add: 'Add Archive',
      addTitle: 'Add Archive',
      editTitle: 'Edit Archive',
      enable: 'Enable',
      disable: 'Disable',
      deleteConfirm: 'Confirm to delete this archive?',
      recordCount: '{total} records'
    },
    recruitment: {
      totalRecruitments: 'Total Recruitments',
      recruiting: 'Recruiting',
      completed: 'Completed',
      cancelled: 'Cancelled',
      positionName: 'Position',
      positionNamePlaceholder: 'Enter position name',
      departmentName: 'Department',
      departmentNamePlaceholder: 'Enter department name',
      recruitNum: 'Headcount',
      recruitNumPlaceholder: 'Enter headcount',
      salaryRange: 'Salary Range',
      salaryRangePlaceholder: 'e.g. 10K-20K',
      recruitStatus: 'Status',
      statusRecruiting: 'Recruiting',
      statusCompleted: 'Completed',
      statusCancelled: 'Cancelled',
      deadline: 'Deadline',
      requirements: 'Requirements',
      requirementsPlaceholder: 'Enter job requirements',
      add: 'Add Recruitment',
      addTitle: 'Add Recruitment',
      editTitle: 'Edit Recruitment',
      enable: 'Enable',
      disable: 'Disable',
      deleteConfirm: 'Confirm to delete this recruitment?',
      recordCount: '{total} records'
    },
    salary: {
      totalRecords: 'Total Records',
      totalBaseSalary: 'Total Base Salary',
      totalNetSalary: 'Total Net Salary',
      employees: 'Employees',
      employeeId: 'Employee ID',
      employeeIdPlaceholder: 'Enter employee ID',
      employee: 'Employee',
      employeePlaceholder: 'Select employee',
      salaryMonth: 'Salary Month',
      salaryMonthPlaceholder: 'Enter salary month',
      salaryMonthFormat: 'Format: YYYY-MM',
      baseSalary: 'Base Salary',
      baseSalaryPlaceholder: 'Enter base salary',
      allowance: 'Allowance',
      allowancePlaceholder: 'Enter allowance',
      overtimePay: 'Overtime Pay',
      overtimePayPlaceholder: 'Enter overtime pay',
      bonus: 'Bonus',
      bonusPlaceholder: 'Enter bonus',
      deduction: 'Deduction',
      deductionPlaceholder: 'Enter deduction',
      grossSalary: 'Gross Salary',
      netSalary: 'Net Salary',
      netSalaryPreview: 'Net Salary Preview',
      paymentStatus: 'Payment Status',
      statusPending: 'Pending',
      statusPaid: 'Paid',
      fiscalYear: 'Fiscal Year',
      fiscalYearPlaceholder: 'Enter fiscal year',
      fiscalMonth: 'Month',
      detailTitle: 'Salary Detail #{id}',
      selectHint: 'Click a row to view details',
      clickRowHint: 'Click a row to view salary details',
      tabBreakdown: 'Breakdown',
      tabSummary: 'Summary',
      tabDetail: 'Salary Items',
      detailItems: 'Salary Detail Items',
      detailItemName: 'Item Name',
      detailItemNamePlaceholder: 'e.g. Base Pay, Performance',
      detailItemType: 'Item Type',
      detailItemTypeAdd: 'Addition',
      detailItemTypeDeduct: 'Deduction',
      detailItemAmount: 'Amount',
      detailItemAmountPlaceholder: 'Enter amount',
      detailAtLeastOne: 'Add at least one salary detail item',
      addDetailRow: 'Add Detail Row',
      add: 'Add Salary',
      addTitle: 'Add Salary Record',
      editTitle: 'Edit Salary Record',
      deleteConfirm: 'Confirm to delete this salary record?',
      recordCount: '{total} records',
      createTime: 'Create Time'
    },
    attendance: {
      totalRecords: 'Attendance Records',
      normal: 'Normal',
      absent: 'Absent',
      overtime: 'Overtime',
      employeeName: 'Employee Name',
      employeeNamePlaceholder: 'Enter employee name',
      employeeId: 'Employee',
      employeeIdPlaceholder: 'Select employee',
      dateRange: 'Date Range',
      attendanceDate: 'Date',
      attendanceType: 'Type',
      checkInTime: 'Check-in Time',
      checkOutTime: 'Check-out Time',
      workHours: 'Work Hours',
      workHoursPlaceholder: 'Enter work hours',
      overtimeHours: 'Overtime Hours',
      overtimeHoursPlaceholder: 'Enter overtime hours',
      typeNormal: 'Normal',
      typeLate: 'Late',
      typeEarly: 'Early',
      typeAbsent: 'Absent',
      typeOvertime: 'Overtime',
      add: 'Add Attendance',
      addTitle: 'Add Attendance',
      editTitle: 'Edit Attendance',
      deleteConfirm: 'Confirm to delete this attendance record?',
      recordCount: '{total} records',
      checkOutAfterCheckIn: 'Check-out time must be after check-in time',
      loadEmployeeFailed: 'Failed to load employee list',
      loadFailed: 'Failed to load attendance list',
      addSuccess: 'Added successfully',
      updateSuccess: 'Updated successfully',
      deleteSuccess: 'Deleted successfully',
      addFailed: 'Add failed',
      updateFailed: 'Update failed',
      deleteFailed: 'Delete failed'
    }
  },
  operate: 'Operation',
  status: 'Status',
  createTime: 'Create Time',
  updateTime: 'Update Time',
  remark: 'Remark',
  desc: 'Description',
  sort: 'Sort',
  enable: 'Enable',
  disable: 'Disable',
  batchDelete: 'Batch Delete',
  more: 'More',
  detail: 'Detail',
  upload: 'Upload',
  download: 'Download',
  preview: 'Preview',
  copy: 'Copy',
  copied: 'Copied',
  tip: 'Tip',
  confirmDelete: 'Confirm to delete selected data?',
  saveSuccess: 'Saved successfully',
  deleteSuccess: 'Deleted successfully',
  operateSuccess: 'Operation successful',
  operateFailed: 'Operation failed'
}

export default common

export type CommonLocale = typeof common
