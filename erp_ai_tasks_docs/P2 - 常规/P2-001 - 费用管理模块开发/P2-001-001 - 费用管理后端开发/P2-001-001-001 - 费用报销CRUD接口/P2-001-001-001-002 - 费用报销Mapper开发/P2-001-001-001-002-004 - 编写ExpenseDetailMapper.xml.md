# P2-001-001-001-002-004 编写ExpenseDetailMapper.xml

## 一、任务标识

| 属性 | 值 |
|------|-----|
| 任务编号 | P2-001-001-001-002-004 |
| 任务名称 | 编写ExpenseDetailMapper.xml |
| 所属模块 | P2-001 |
| 优先级 | P2 |
| 任务类型 | Mapper数据访问层 |
...
编写ExpenseDetailMapper.xml：resultMap定义；自定义SQL(主从JOIN/动态条件<if>/<choose>)；分页使用MyBatis-Plus Page；#{}参数绑定防注入
...
- P2-001-001-001-002-003 编写ExpenseMapper.xml（前序兄弟任务）
...
<mapper namespace="com.xxx.expense.mapper.ExpenseDetailMapper">
    <resultMap id="BaseResultMap" type="com.xxx.expense.entity.ExpenseDetailEntity">
...
| 序号 | 文件路径 | 说明 |
|:---:|---------|------|
| 1 | src/main/resources/mapper/expense/ExpenseDetailMapper.xml | ExpenseDetailMapper XML |
