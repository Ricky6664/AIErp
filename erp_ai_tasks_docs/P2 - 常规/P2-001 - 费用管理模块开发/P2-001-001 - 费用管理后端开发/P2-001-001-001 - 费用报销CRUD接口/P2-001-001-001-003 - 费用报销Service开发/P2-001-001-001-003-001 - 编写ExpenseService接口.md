# P2-001-001-001-003-001 编写ExpenseService接口

## 一、任务标识

| 属性 | 值 |
|------|-----|
| 任务编号 | P2-001-001-001-003-001 |
| 任务名称 | 编写ExpenseService接口 |
| 所属模块 | P2-001 |
| 优先级 | P2 |
| 任务类型 | Service服务层 |
...
定义ExpenseService接口：extends IServiceX<ExpenseEntity>；CRUD方法(create/update/delete/getDetail/page)+业务方法(submit/checkBudget等)
...
```java
public interface ExpenseService extends IServiceX<ExpenseEntity> {
    ExpenseVO create(ExpenseCreateDTO dto);
    Boolean update(ExpenseUpdateDTO dto);
    Boolean delete(Long id);
    ExpenseVO getDetail(Long id);
    Page<ExpenseListVO> getPage(ExpenseQueryDTO query);
...
| 序号 | 文件路径 | 说明 |
|:---:|---------|------|
| 1 | src/main/java/com/erp/expense/service/ExpenseService.java | ExpenseService接口 |
