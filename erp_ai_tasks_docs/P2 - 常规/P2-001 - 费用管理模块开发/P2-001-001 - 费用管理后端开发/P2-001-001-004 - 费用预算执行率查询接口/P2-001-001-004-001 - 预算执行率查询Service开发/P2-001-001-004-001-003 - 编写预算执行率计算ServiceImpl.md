...
| 属性 | 值 |
|------|-----|
| 任务编号 | P2-001-001-004-001-003 |
| 任务名称 | 编写ExpenseBudgetExecutionServiceImpl实现类（@Service+执行率计算逻辑：已用金额=SUM(已申请金额)+SUM(已报销金额)，剩余金额=预算金额-已用金额，执行率=已用金额/预算金额×100%+按费用项目GROUP BY汇总+按部门GROUP BY汇总+按期间GROUP BY趋势计算+缓存@Cacheable("budgetExecution")+@Transactional(readOnly=true)） |
| 所属模块 | P2-001 |
| 优先级 | P2 |
| 任务类型 | Service服务层 |
...
实现ExpenseBudgetExecutionServiceImpl：extends ServiceImplX；@Service+@Transactional事务；DTO↔Entity转换(MapStruct)；业务校验+编码生成+持久化+操作日志
...
- P2-001-001-004-001-002 编写ExpenseBudgetExecutionService接口（前序兄弟任务）
...
@Slf4j
@Service
public class ExpenseBudgetExecutionServiceImpl extends ServiceImplX<ExpenseBudgetExecutionMapper, ExpenseBudgetExecutionEntity> implements ExpenseBudgetExecutionService {
    @Autowired private ExpenseBudgetExecutionMapper expenseBudgetExecutionMapper;
    @Autowired private CodeGeneratorService codeGeneratorService;

    @Override
    @Transactional(rollbackFor = Exception.class)
    public ExpenseBudgetExecutionVO create(ExpenseBudgetExecutionCreateDTO dto) {
        validateUnique(dto.getCode(), null);
        ExpenseBudgetExecutionEntity entity = MapStructConverter.INSTANCE.toEntity(dto);
        entity.setCode(codeGeneratorService.generate("expense.expense_budget_execution"));
        entity.setStatus(0);
        save(entity);
...
| 序号 | 文件路径 | 说明 |
|:---:|---------|------|
| 1 | src/main/java/com/erp/expense/service/impl/ExpenseBudgetExecutionServiceImpl.java | ExpenseBudgetExecutionService实现 |
