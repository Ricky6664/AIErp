# P2-001-001-004-001-001 编写ExpenseBudgetExecutionRateVO

## 一、任务标识

| 属性 | 值 |
|------|-----|
| 任务编号 | P2-001-001-004-001-001 |
| 任务名称 | 编写ExpenseBudgetExecutionRateVO |
| 所属模块 | P2-001 |
| 优先级 | P2 |
...
定义ExpenseBudgetExecutionRateVO详情对象：完整业务字段+关联翻译(deptName/userName)；@JsonFormat日期格式化；字典翻译字段(statusName)；BigDecimal金额精度
...
```java
@Data
public class ExpenseBudgetExecutionRateVO {
    private Long id;
    private String code;
    private String name;
    ...
    private List<ExpenseBudgetExecutionRateDetailVO> details;
}
```
...
| 序号 | 文件路径 | 说明 |
|:---:|---------|------|
| 1 | src/main/java/com/erp/expense/vo/ExpenseBudgetExecutionRateVO.java | ExpenseBudgetExecutionRateVO |
