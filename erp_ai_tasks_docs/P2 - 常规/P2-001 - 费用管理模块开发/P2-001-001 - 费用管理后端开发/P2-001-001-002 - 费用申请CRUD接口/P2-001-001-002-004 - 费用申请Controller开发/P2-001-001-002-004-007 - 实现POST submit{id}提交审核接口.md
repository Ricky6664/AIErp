# P2-001-001-002-004-007 实现POST /submit/{id}提交审核接口

## 一、任务标识

| 属性 | 值 |
|------|-----|
| 任务编号 | P2-001-001-002-004-007 |
...
实现POST /submit/{id}提交审核Expense接口：校验草稿状态；更新status=已提交；发起审批流程；记录操作日志；发送待办通知
...
- P2-001-001-002-004-006 实现GET /detail/{id}详情查询接口（前序兄弟任务）
...
### 5.1 接口
- URL: POST /api/expense/expense/submit/{id}
