# P2-001-001-002-001-002 编写ExpenseApplicationDetailEntity类

## 一、任务标识

| 属性 | 值 |
|------|-----|
| 任务编号 | P2-001-001-002-001-002 |
| 任务名称 | 编写ExpenseApplicationDetailEntity类 |
| 所属模块 | P2-001 |
| 优先级 | P2 |
...
定义ExpenseApplicationEntity类：@TableName("expense_expense_application") + @TableId(type=IdType.ASSIGN_ID)雪花ID策略；业务字段使用@TableField显式映射；公共字段(create_by/create_time/update_by/update_time/tenant_id)通过MetaHandler自动填充；@TableLogic逻辑删除is_deleted默认0；@Version乐观锁version字段；implements Serializable
...
- P2-001-001-002-001-001 编写ExpenseApplicationEntity类（前序兄弟任务）
...
```java
@Data
@TableName("expense_expense_application")
public class ExpenseApplicationEntity implements Serializable {
...
| 序号 | 文件路径 | 说明 |
|:---:|---------|------|
| 1 | src/main/java/com/erp/expense/entity/ExpenseApplicationEntity.java | ExpenseApplication实体类 |
...
| 序号 | 检查项 | 验证方法 |
|:---:|--------|---------|
| 1 | @TableName("expense_expense_application")正确 | 启动测试 |
