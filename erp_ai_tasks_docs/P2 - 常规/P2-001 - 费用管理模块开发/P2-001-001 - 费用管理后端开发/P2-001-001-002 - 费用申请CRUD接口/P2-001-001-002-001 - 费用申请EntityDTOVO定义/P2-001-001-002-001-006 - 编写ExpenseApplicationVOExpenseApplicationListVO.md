# P2-001-001-002-001-006 编写ExpenseApplicationVO/ExpenseApplicationListVO

## 一、任务标识

| 属性 | 值 |
|------|-----|
| 任务编号 | P2-001-001-002-001-006 |
| 任务名称 | 编写ExpenseApplicationVO/ExpenseApplicationListVO |
| 所属模块 | P2-001 |
| 优先级 | P2 |
...
定义ExpenseApplicationVO详情对象：完整业务字段+关联翻译(deptName/userName)；@JsonFormat日期格式化；字典翻译字段(statusName)；BigDecimal金额精度
...
- P2-001-001-002-001-005 编写ExpenseApplicationQueryDTO（前序兄弟任务）
...
public class ExpenseApplicationVO {
    ...
    private List<ExpenseApplicationDetailVO> details;
}
```
...
| 序号 | 文件路径 | 说明 |
|:---:|---------|------|
| 1 | src/main/java/com/erp/expense/vo/ExpenseApplicationVO.java | ExpenseApplicationVO |
