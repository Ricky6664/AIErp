# P2-001-001-002-001-003 编写ExpenseApplicationCreateDTO

## 一、任务标识

| 属性 | 值 |
|------|-----|
| 任务编号 | P2-001-001-002-001-003 |
| 任务名称 | 编写ExpenseApplicationCreateDTO |
| 所属模块 | P2-001 |
| 优先级 | P2 |
...
定义ExpenseApplicationCreateDTO：@NotBlank标注必填字符串(编号/名称)；@NotNull标注必填引用字段(部门ID/币种ID)；@Size限制字段长度；@DecimalMin金额≥0；嵌套明细列表@Valid+@NotEmpty级联校验；@DateTimeFormat日期格式
...
- P2-001-001-002-001-002 编写ExpenseApplicationDetailEntity类（前序兄弟任务）
...
```java
@Data
public class ExpenseApplicationCreateDTO {
    ...
    private List<ExpenseApplicationDetailDTO> details;
...
| 序号 | 文件路径 | 说明 |
|:---:|---------|------|
| 1 | src/main/java/com/erp/expense/dto/ExpenseApplicationCreateDTO.java | ExpenseApplication创建DTO |
