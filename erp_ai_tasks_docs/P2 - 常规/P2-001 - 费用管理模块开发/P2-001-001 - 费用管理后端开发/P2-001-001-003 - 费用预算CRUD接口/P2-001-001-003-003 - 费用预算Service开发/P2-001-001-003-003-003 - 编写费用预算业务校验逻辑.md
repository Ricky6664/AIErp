...
| 属性 | 值 |
|------|-----|
| 任务编号 | P2-001-001-003-003-003 |
| 任务名称 | 业务校验逻辑（预算期间唯一性校验（年度+期间+部门+费用项目四维度唯一）+预算金额精度校验（保留两位小数）+预算期间不可跨年+已执行预算不可删除+budget_no唯一性校验+预算期间必须为已定义的会计期间） |
| 所属模块 | P2-001 |
| 优先级 | P2 |
| 任务类型 | 验证/测试 |
