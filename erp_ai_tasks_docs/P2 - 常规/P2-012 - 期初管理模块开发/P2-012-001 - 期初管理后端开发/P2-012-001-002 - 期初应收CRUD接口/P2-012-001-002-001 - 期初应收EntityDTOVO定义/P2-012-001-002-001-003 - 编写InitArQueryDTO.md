# P2-012-001-002-001-003 编写InitArQueryDTO

## 一、任务标识

| 属性 | 值 |
|------|-----|
| 任务编号 | P2-012-001-002-001-003 |
| 任务名称 | 编写InitArQueryDTO |
| 所属模块 | P2-012 |
| 优先级 | P2 |
...
定义编写InitArQueryDTO：CreateDTO使用@NotBlank/@NotNull/@DecimalMin等校验注解确保必填字段非空、数值字段范围合法；UpdateDTO继承CreateDTO并增加id字段；QueryDTO包含分页参数(pageNum/pageSize)+筛选条件字段+排序字段(sortField/sortOrder)
...
- P2-012-001-002-001-002 编写InitArCreateDTO（@NotNull客户+@NotNull应收日期+@DecimalMin("0.01")应收金额>0+币种+应收类型+账期天数+到期日自动计算=应收日期+账期天数+校验规则：客户维度汇总不允许同一客户同一订单号重复）（前序兄弟任务）
...
| 字段名 | 类型 | 校验注解 | 说明 |
|--------|------|---------|------|
| (按编写InitArQueryDTO列出必填字段) | - | @NotBlank/@NotNull | 必填字段 |
...
| 序号 | 文件路径 | 说明 |
|:---:|---------|------|
| 1 | src/main/java/com/erp/init/dto/InitArQueryDTO.java | 查询DTO |
