# P2-012-001-002-001-004 编写InitArVO/InitArListVO

## 一、任务标识

| 属性 | 值 |
|------|-----|
| 任务编号 | P2-012-001-002-001-004 |
| 任务名称 | 编写InitArVO/InitArListVO |
| 所属模块 | P2-012 |
| 优先级 | P2 |
...
定义编写InitArVO/InitArListVO：ListVO仅含列表展示必要字段（含id/核心业务字段/status/createdAt）+字典翻译字段（xxxName后缀）；DetailVO含全部字段+关联对象；日期字段使用@JsonFormat(pattern="yyyy-MM-dd HH:mm:ss")格式化

## 三、前置依赖

### 3.1 前置任务

- P2-012-001-002-001 期初应收Entity/DTO/VO定义（父任务）
- P2-012-001-002-001-003 编写InitArQueryDTO（前序兄弟任务）
