...
| 属性 | 值 |
|------|-----|
| 任务编号 | P2-012-001-006-003-002 |
| 任务名称 | 编写InitFaAssetServiceImpl实现类（@Service+期初编号自动生成调用SerialNumberService.generate("QCZC")+净值自动计算：net_value=original_value-accumulated_depreciation+月折旧额按折旧方法自动计算+与P2-002相同的四种折旧方法公式+approve后writeToFaAsset方法：①在资产卡片表fa_asset中创建资产记录②原值/累计折旧/净值/月折旧额等字段直接写入③资产状态设置为"使用中"+资产编号唯一性校验：不能与已存在资产卡片编号重复+@Transactional事务管理） |
| 所属模块 | P2-012 |
| 优先级 | P2 |
| 任务类型 | Service服务层 |
...
- P2-012-001-006-003-001 编写InitFaAssetService接口（前序兄弟任务）
