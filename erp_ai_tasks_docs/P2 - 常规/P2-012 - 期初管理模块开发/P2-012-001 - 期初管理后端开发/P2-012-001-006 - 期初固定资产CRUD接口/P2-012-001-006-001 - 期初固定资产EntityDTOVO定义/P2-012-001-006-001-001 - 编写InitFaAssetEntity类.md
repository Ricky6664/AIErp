# P2-012-001-006-001-001 编写InitFaAssetEntity类

## 一、任务标识

| 属性 | 值 |
|------|-----|
| 任务编号 | P2-012-001-006-001-001 |
| 任务名称 | 编写InitFaAssetEntity类 |
| 所属模块 | P2-012 |
| 优先级 | P2 |
...
定义InitFaAssetEntity类：@TableName("init_inventory")，@TableId(type=IdType.ASSIGN_ID)雪花ID，继承BaseEntity获取10个通用字段，业务字段按驼峰命名映射数据库下划线字段，@TableLogic标注isDeleted逻辑删除，@Version标注version乐观锁
...
| (按编写InitFaAssetEntity类补充业务字段) | - | @TableField | - | - |
...
public class InitFaAssetEntity extends BaseEntity {
    // 业务字段按编写InitFaAssetEntity类定义
}
```
...
| 序号 | 文件路径 | 说明 |
|:---:|---------|------|
| 1 | src/main/java/com/erp/init/entity/InitFaAssetEntity.java | Entity实体类 |
