...
| 属性 | 值 |
|------|-----|
| 任务编号 | P2-012-001-006-001-002 |
| 任务名称 | 编写InitFaAssetCreateDTO（@NotBlank资产编号+@NotBlank资产名称+@NotNull资产分类+@NotNull取得日期+@DecimalMin("0.01")原值>0+残值率0-100+@NotNull使用年限+@NotNull已使用年限+@NotBlank折旧方法+期初净值自动计算=原值-累计折旧+已使用年限<使用年限校验） |
| 所属模块 | P2-012 |
| 优先级 | P2 |
...
定义编写InitFaAssetCreateDTO（@NotBlank资产编号+@NotBlank资产名称+@NotNull资产分类+@NotNull取得日期+@DecimalMin("0.01")原值>0+残值率0-100+@NotNull使用年限+@NotNull已使用年限+@NotBlank折旧方法+期初净值自动计算=原值-累计折旧+已使用年限<使用年限校验）：CreateDTO使用@NotBlank/@NotNull/@DecimalMin等校验注解确保必填字段非空、数值字段范围合法；UpdateDTO继承CreateDTO并增加id字段；QueryDTO包含分页参数(pageNum/pageSize)+筛选条件字段+排序字段(sortField/sortOrder)

## 三、前置依赖

### 3.1 前置任务

- P2-012-001-006-001 期初固定资产Entity/DTO/VO定义（父任务）
- P2-012-001-006-001-001 编写InitFaAssetEntity类（前序兄弟任务）
...
| 字段名 | 类型 | 校验注解 | 说明 |
|--------|------|---------|------|
| (按编写InitFaAssetCreateDTO（@NotBlank资产编号+@NotBlank资产名称+@NotNull资产分类+@NotNull取得日期+@DecimalMin("0.01")原值>0+残值率0-100+@NotNull使用年限+@NotNull已使用年限+@NotBlank折旧方法+期初净值自动计算=原值-累计折旧+已使用年限<使用年限校验）列出必填字段) | - | @NotBlank/@NotNull | 必填字段 |
