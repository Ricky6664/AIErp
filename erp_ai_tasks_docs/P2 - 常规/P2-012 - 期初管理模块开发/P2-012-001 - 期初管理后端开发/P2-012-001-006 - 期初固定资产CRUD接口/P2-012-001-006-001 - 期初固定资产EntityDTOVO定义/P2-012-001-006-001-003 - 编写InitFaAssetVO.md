# P2-012-001-006-001-003 编写InitFaAssetVO

## 一、任务标识

| 属性 | 值 |
|------|-----|
| 任务编号 | P2-012-001-006-001-003 |
| 任务名称 | 编写InitFaAssetVO |
| 所属模块 | P2-012 |
| 优先级 | P2 |
...
定义编写InitFaAssetVO：ListVO仅含列表展示必要字段（含id/核心业务字段/status/createdAt）+字典翻译字段（xxxName后缀）；DetailVO含全部字段+关联对象；日期字段使用@JsonFormat(pattern="yyyy-MM-dd HH:mm:ss")格式化
...
- P2-012-001-006-001-002 编写InitFaAssetCreateDTO（@NotBlank资产编号+@NotBlank资产名称+@NotNull资产分类+@NotNull取得日期+@DecimalMin("0.01")原值>0+残值率0-100+@NotNull使用年限+@NotNull已使用年限+@NotBlank折旧方法+期初净值自动计算=原值-累计折旧+已使用年限<使用年限校验）（前序兄弟任务）
...
| 序号 | 文件路径 | 说明 |
|:---:|---------|------|
| 1 | src/main/java/com/erp/init/vo/InitFaAssetDetailVO.java | 详情VO |
