...
| 属性 | 值 |
|------|-----|
| 任务编号 | P2-012-001-001-001-003 |
| 任务名称 | 编写InitInventoryCreateDTO（@NotNull仓库+@NotNull启用日期+@NotEmpty明细列表+校验规则：仓库+商品维度唯一性（同仓库同商品不可重复）+@DecimalMin数量>0+单价>=0+金额=数量×单价自动计算校验） |
| 所属模块 | P2-012 |
| 优先级 | P2 |
...
定义编写InitInventoryCreateDTO（@NotNull仓库+@NotNull启用日期+@NotEmpty明细列表+校验规则：仓库+商品维度唯一性（同仓库同商品不可重复）+@DecimalMin数量>0+单价>=0+金额=数量×单价自动计算校验）：CreateDTO使用@NotBlank/@NotNull/@DecimalMin等校验注解确保必填字段非空、数值字段范围合法；UpdateDTO继承CreateDTO并增加id字段；QueryDTO包含分页参数(pageNum/pageSize)+筛选条件字段+排序字段(sortField/sortOrder)

## 三、前置依赖

### 3.1 前置任务

- P2-012-001-001-001 期初库存Entity/DTO/VO定义（父任务）
- P2-012-001-001-001-002 编写InitInventoryDetailEntity类（前序兄弟任务）
...
| 字段名 | 类型 | 校验注解 | 说明 |
|--------|------|---------|------|
| (按编写InitInventoryCreateDTO（@NotNull仓库+@NotNull启用日期+@NotEmpty明细列表+校验规则：仓库+商品维度唯一性（同仓库同商品不可重复）+@DecimalMin数量>0+单价>=0+金额=数量×单价自动计算校验）列出必填字段) | - | @NotBlank/@NotNull | 必填字段 |
