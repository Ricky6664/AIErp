# P2-012-001-001-001-004 编写InitInventoryQueryDTO

## 一、任务标识

| 属性 | 值 |
|------|-----|
| 任务编号 | P2-012-001-001-001-004 |
| 任务名称 | 编写InitInventoryQueryDTO |
| 所属模块 | P2-012 |
| 优先级 | P2 |
...
定义编写InitInventoryQueryDTO：CreateDTO使用@NotBlank/@NotNull/@DecimalMin等校验注解确保必填字段非空、数值字段范围合法；UpdateDTO继承CreateDTO并增加id字段；QueryDTO包含分页参数(pageNum/pageSize)+筛选条件字段+排序字段(sortField/sortOrder)
...
- P2-012-001-001-001-003 编写InitInventoryCreateDTO（@NotNull仓库+@NotNull启用日期+@NotEmpty明细列表+校验规则：仓库+商品维度唯一性（同仓库同商品不可重复）+@DecimalMin数量>0+单价>=0+金额=数量×单价自动计算校验）（前序兄弟任务）
...
| 字段名 | 类型 | 校验注解 | 说明 |
|--------|------|---------|------|
| (按编写InitInventoryQueryDTO列出必填字段) | - | @NotBlank/@NotNull | 必填字段 |
...
| 序号 | 文件路径 | 说明 |
|:---:|---------|------|
| 1 | src/main/java/com/erp/init/dto/InitInventoryQueryDTO.java | 查询DTO |
