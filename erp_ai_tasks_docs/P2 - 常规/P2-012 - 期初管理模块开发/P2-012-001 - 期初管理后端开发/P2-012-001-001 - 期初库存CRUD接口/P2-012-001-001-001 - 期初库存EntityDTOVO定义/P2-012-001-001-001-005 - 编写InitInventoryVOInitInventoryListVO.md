# P2-012-001-001-001-005 编写InitInventoryVO/InitInventoryListVO

## 一、任务标识

| 属性 | 值 |
|------|-----|
| 任务编号 | P2-012-001-001-001-005 |
| 任务名称 | 编写InitInventoryVO/InitInventoryListVO |
| 所属模块 | P2-012 |
| 优先级 | P2 |
...
定义编写InitInventoryVO/InitInventoryListVO：ListVO仅含列表展示必要字段（含id/核心业务字段/status/createdAt）+字典翻译字段（xxxName后缀）；DetailVO含全部字段+关联对象；日期字段使用@JsonFormat(pattern="yyyy-MM-dd HH:mm:ss")格式化
...
- P2-012-001-001-001-004 编写InitInventoryQueryDTO（前序兄弟任务）
