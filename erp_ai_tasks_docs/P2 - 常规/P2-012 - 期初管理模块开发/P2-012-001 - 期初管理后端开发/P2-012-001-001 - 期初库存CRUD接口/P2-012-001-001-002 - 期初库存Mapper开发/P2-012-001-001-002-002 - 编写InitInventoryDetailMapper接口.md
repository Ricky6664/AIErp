# P2-012-001-001-002-002 编写InitInventoryDetailMapper接口

## 一、任务标识

| 属性 | 值 |
|------|-----|
| 任务编号 | P2-012-001-001-002-002 |
| 任务名称 | 编写InitInventoryDetailMapper接口 |
| 所属模块 | P2-012 |
| 优先级 | P2 |
| 任务类型 | Mapper数据访问层 |
...
编写InitInventoryDetailMapper接口：继承BaseMapper<Entity>，定义自定义查询方法（复杂联表/统计/批量操作），对应XML映射文件编写SQL，所有查询自动追加is_deleted=false条件，多租户表自动追加tenant_id条件
...
- P2-012-001-001-002-001 编写InitInventoryMapper接口（extends BaseMapperX<InitInventoryEntity>+@Mapper+selectByWarehouse按仓库查询+selectPageByCondition条件分页+selectByUniqueKey唯一性校验查询（仓库+商品））（前序兄弟任务）
...
| 序号 | 文件路径 | 说明 |
|:---:|---------|------|
| 1 | src/main/java/com/erp/init/mapper/InitInventoryDetailMapper接口Mapper.java | Mapper接口 |
| 2 | src/main/resources/mapper/init/InitInventoryDetailMapper接口Mapper.xml | Mapper XML |
