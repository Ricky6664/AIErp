...
| 属性 | 值 |
|------|-----|
| 任务编号 | P2-012-001-004-001-002 |
| 任务名称 | 编写InitArInvoiceCreateDTO（@NotNull客户+@NotNull商品+@DecimalMin("0.01")金额>0+@DecimalMin("0")税率+税额自动计算=金额×税率+价税合计自动计算=金额+税额+校验规则：客户+商品+来源单据号维度唯一性） |
| 所属模块 | P2-012 |
| 优先级 | P2 |
...
定义编写InitArInvoiceCreateDTO（@NotNull客户+@NotNull商品+@DecimalMin("0.01")金额>0+@DecimalMin("0")税率+税额自动计算=金额×税率+价税合计自动计算=金额+税额+校验规则：客户+商品+来源单据号维度唯一性）：CreateDTO使用@NotBlank/@NotNull/@DecimalMin等校验注解确保必填字段非空、数值字段范围合法；UpdateDTO继承CreateDTO并增加id字段；QueryDTO包含分页参数(pageNum/pageSize)+筛选条件字段+排序字段(sortField/sortOrder)

## 三、前置依赖

### 3.1 前置任务

- P2-012-001-004-001 期初应开票Entity/DTO/VO定义（父任务）
- P2-012-001-004-001-001 编写InitArInvoiceEntity类（前序兄弟任务）
...
| 字段名 | 类型 | 校验注解 | 说明 |
|--------|------|---------|------|
| (按编写InitArInvoiceCreateDTO（@NotNull客户+@NotNull商品+@DecimalMin("0.01")金额>0+@DecimalMin("0")税率+税额自动计算=金额×税率+价税合计自动计算=金额+税额+校验规则：客户+商品+来源单据号维度唯一性）列出必填字段) | - | @NotBlank/@NotNull | 必填字段 |
