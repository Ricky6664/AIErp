# P2-012-001-004-001-003 编写InitArInvoiceQueryDTO

## 一、任务标识

| 属性 | 值 |
|------|-----|
| 任务编号 | P2-012-001-004-001-003 |
| 任务名称 | 编写InitArInvoiceQueryDTO |
| 所属模块 | P2-012 |
| 优先级 | P2 |
...
定义编写InitArInvoiceQueryDTO：CreateDTO使用@NotBlank/@NotNull/@DecimalMin等校验注解确保必填字段非空、数值字段范围合法；UpdateDTO继承CreateDTO并增加id字段；QueryDTO包含分页参数(pageNum/pageSize)+筛选条件字段+排序字段(sortField/sortOrder)
...
- P2-012-001-004-001-002 编写InitArInvoiceCreateDTO（@NotNull客户+@NotNull商品+@DecimalMin("0.01")金额>0+@DecimalMin("0")税率+税额自动计算=金额×税率+价税合计自动计算=金额+税额+校验规则：客户+商品+来源单据号维度唯一性）（前序兄弟任务）
...
| 字段名 | 类型 | 校验注解 | 说明 |
|--------|------|---------|------|
| (按编写InitArInvoiceQueryDTO列出必填字段) | - | @NotBlank/@NotNull | 必填字段 |
...
| 序号 | 文件路径 | 说明 |
|:---:|---------|------|
| 1 | src/main/java/com/erp/init/dto/InitArInvoiceQueryDTO.java | 查询DTO |
