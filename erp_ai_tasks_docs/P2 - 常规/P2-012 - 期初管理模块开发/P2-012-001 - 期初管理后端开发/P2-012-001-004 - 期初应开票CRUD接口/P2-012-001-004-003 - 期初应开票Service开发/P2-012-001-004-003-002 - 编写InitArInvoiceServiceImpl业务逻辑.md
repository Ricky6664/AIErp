...
| 属性 | 值 |
|------|-----|
| 任务编号 | P2-012-001-004-003-002 |
| 任务名称 | 编写InitArInvoiceServiceImpl实现类（@Service+期初编号自动生成调用SerialNumberService.generate("QCYKP")+税额自动计算：tax_amount=amount×tax_rate+价税合计自动计算：total_amount=amount+tax_amount+approve后writeToArInvoice方法：在应开票表ar_invoice中插入记录，已开票=0，未开票=价税合计+已发生开票业务的客户期初不可修改+@Transactional事务管理） |
| 所属模块 | P2-012 |
| 优先级 | P2 |
...
定义编写InitArInvoiceServiceImpl实现类（@Service+期初编号自动生成调用SerialNumberService.generate("QCYKP")+税额自动计算：tax_amount=amount×tax_rate+价税合计自动计算：total_amount=amount+tax_amount+approve后writeToArInvoice方法：在应开票表ar_invoice中插入记录，已开票=0，未开票=价税合计+已发生开票业务的客户期初不可修改+@Transactional事务管理）：Entity/DTO/VO三类模型完整定义，字段与数据库表一一对应，校验注解完备，展示字段含字典翻译
...
- P2-012-001-004-003-001 编写InitArInvoiceService接口（前序兄弟任务）
...
| 2 | src/main/java/com/erp/init/dto/InitCreateDTO.java | DTO类 |
| 3 | src/main/java/com/erp/init/vo/InitListVO.java | VO类 |
