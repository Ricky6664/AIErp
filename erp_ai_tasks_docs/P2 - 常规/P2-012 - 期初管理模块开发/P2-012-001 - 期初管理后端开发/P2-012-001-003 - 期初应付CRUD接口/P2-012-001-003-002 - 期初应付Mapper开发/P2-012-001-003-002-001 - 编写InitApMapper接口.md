# P2-012-001-003-002-001 编写InitApMapper接口

## 一、任务标识

| 属性 | 值 |
|------|-----|
| 任务编号 | P2-012-001-003-002-001 |
| 任务名称 | 编写InitApMapper接口 |
| 所属模块 | P2-012 |
| 优先级 | P2 |
| 任务类型 | Mapper数据访问层 |
...
编写InitApMapper接口：继承BaseMapper<Entity>，定义自定义查询方法（复杂联表/统计/批量操作），对应XML映射文件编写SQL，所有查询自动追加is_deleted=false条件，多租户表自动追加tenant_id条件
...
| 序号 | 文件路径 | 说明 |
|:---:|---------|------|
| 1 | src/main/java/com/erp/init/mapper/InitApMapper接口Mapper.java | Mapper接口 |
| 2 | src/main/resources/mapper/init/InitApMapper接口Mapper.xml | Mapper XML |
