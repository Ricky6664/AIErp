# P2-012-001-007-001-003 编写InitAccountBalanceVO/InitAccountBalanceListVO

## 一、任务标识

| 属性 | 值 |
|------|-----|
| 任务编号 | P2-012-001-007-001-003 |
| 任务名称 | 编写InitAccountBalanceVO/InitAccountBalanceListVO |
| 所属模块 | P2-012 |
| 优先级 | P2 |
...
定义编写InitAccountBalanceVO/InitAccountBalanceListVO：ListVO仅含列表展示必要字段（含id/核心业务字段/status/createdAt）+字典翻译字段（xxxName后缀）；DetailVO含全部字段+关联对象；日期字段使用@JsonFormat(pattern="yyyy-MM-dd HH:mm:ss")格式化
...
- P2-012-001-007-001-002 编写InitAccountBalanceCreateDTO（@NotNull会计年度+@NotNull会计期间+@NotNull科目+@DecimalMin("0")期初借方金额>=0+@DecimalMin("0")期初贷方金额>=0+币种+辅助核算可选+校验规则：借方贷方至少一方为0（同一科目不可同时有借有贷）+科目+辅助核算维度唯一性+试算平衡校验（全部借方合计=全部贷方合计））（前序兄弟任务）
...
| 序号 | 文件路径 | 说明 |
|:---:|---------|------|
| 1 | src/main/java/com/erp/init/vo/InitListVO.java | 列表VO |
