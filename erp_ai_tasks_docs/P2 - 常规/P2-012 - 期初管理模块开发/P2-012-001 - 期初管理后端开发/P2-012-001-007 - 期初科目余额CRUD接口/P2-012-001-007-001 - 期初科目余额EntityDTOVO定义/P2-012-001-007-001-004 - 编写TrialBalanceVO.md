# P2-012-001-007-001-004 编写TrialBalanceVO

## 一、任务标识

| 属性 | 值 |
|------|-----|
| 任务编号 | P2-012-001-007-001-004 |
| 任务名称 | 编写TrialBalanceVO |
| 所属模块 | P2-012 |
| 优先级 | P2 |
...
定义编写TrialBalanceVO：ListVO仅含列表展示必要字段（含id/核心业务字段/status/createdAt）+字典翻译字段（xxxName后缀）；DetailVO含全部字段+关联对象；日期字段使用@JsonFormat(pattern="yyyy-MM-dd HH:mm:ss")格式化
...
- P2-012-001-007-001-003 编写InitAccountBalanceVO/InitAccountBalanceListVO（前序兄弟任务）
...
| 序号 | 文件路径 | 说明 |
|:---:|---------|------|
| 1 | src/main/java/com/erp/init/vo/TrialBalanceDetailVO.java | 详情VO |
