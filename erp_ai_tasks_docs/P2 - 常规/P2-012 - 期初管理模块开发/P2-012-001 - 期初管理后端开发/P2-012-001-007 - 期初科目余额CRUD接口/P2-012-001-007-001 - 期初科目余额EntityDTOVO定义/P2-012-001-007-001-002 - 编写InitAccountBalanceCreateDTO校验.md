...
| 属性 | 值 |
|------|-----|
| 任务编号 | P2-012-001-007-001-002 |
| 任务名称 | 编写InitAccountBalanceCreateDTO（@NotNull会计年度+@NotNull会计期间+@NotNull科目+@DecimalMin("0")期初借方金额>=0+@DecimalMin("0")期初贷方金额>=0+币种+辅助核算可选+校验规则：借方贷方至少一方为0（同一科目不可同时有借有贷）+科目+辅助核算维度唯一性+试算平衡校验（全部借方合计=全部贷方合计）） |
| 所属模块 | P2-012 |
| 优先级 | P2 |
...
定义编写InitAccountBalanceCreateDTO（@NotNull会计年度+@NotNull会计期间+@NotNull科目+@DecimalMin("0")期初借方金额>=0+@DecimalMin("0")期初贷方金额>=0+币种+辅助核算可选+校验规则：借方贷方至少一方为0（同一科目不可同时有借有贷）+科目+辅助核算维度唯一性+试算平衡校验（全部借方合计=全部贷方合计））：CreateDTO使用@NotBlank/@NotNull/@DecimalMin等校验注解确保必填字段非空、数值字段范围合法；UpdateDTO继承CreateDTO并增加id字段；QueryDTO包含分页参数(pageNum/pageSize)+筛选条件字段+排序字段(sortField/sortOrder)
...
| 字段名 | 类型 | 校验注解 | 说明 |
|--------|------|---------|------|
| (按编写InitAccountBalanceCreateDTO（@NotNull会计年度+@NotNull会计期间+@NotNull科目+@DecimalMin("0")期初借方金额>=0+@DecimalMin("0")期初贷方金额>=0+币种+辅助核算可选+校验规则：借方贷方至少一方为0（同一科目不可同时有借有贷）+科目+辅助核算维度唯一性+试算平衡校验（全部借方合计=全部贷方合计））列出必填字段) | - | @NotBlank/@NotNull | 必填字段 |
