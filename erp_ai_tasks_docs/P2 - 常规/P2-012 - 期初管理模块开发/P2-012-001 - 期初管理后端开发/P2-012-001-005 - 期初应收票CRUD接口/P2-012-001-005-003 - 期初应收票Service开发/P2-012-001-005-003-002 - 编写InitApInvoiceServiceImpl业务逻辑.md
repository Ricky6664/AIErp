...
| 属性 | 值 |
|------|-----|
| 任务编号 | P2-012-001-005-003-002 |
| 任务名称 | 编写InitApInvoiceServiceImpl实现类（@Service+期初编号自动生成调用SerialNumberService.generate("QCYSP")+税额自动计算+价税合计自动计算+approve后writeToApInvoice方法：在应收票表ap_invoice中插入记录+已启用供应商不可修改+@Transactional事务管理） |
| 所属模块 | P2-012 |
| 优先级 | P2 |
...
定义编写InitApInvoiceServiceImpl实现类（@Service+期初编号自动生成调用SerialNumberService.generate("QCYSP")+税额自动计算+价税合计自动计算+approve后writeToApInvoice方法：在应收票表ap_invoice中插入记录+已启用供应商不可修改+@Transactional事务管理）：Entity/DTO/VO三类模型完整定义，字段与数据库表一一对应，校验注解完备，展示字段含字典翻译

## 三、前置依赖

### 3.1 前置任务

- P2-012-001-005-003 期初应收票Service开发（父任务）
- P2-012-001-005-003-001 编写InitApInvoiceService接口（前序兄弟任务）
