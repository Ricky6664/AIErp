# P2-012-001-005-001-001 编写InitApInvoiceEntity类

## 一、任务标识

| 属性 | 值 |
|------|-----|
| 任务编号 | P2-012-001-005-001-001 |
| 任务名称 | 编写InitApInvoiceEntity类 |
| 所属模块 | P2-012 |
| 优先级 | P2 |
...
定义InitApInvoiceEntity类：@TableName("init_inventory")，@TableId(type=IdType.ASSIGN_ID)雪花ID，继承BaseEntity获取10个通用字段，业务字段按驼峰命名映射数据库下划线字段，@TableLogic标注isDeleted逻辑删除，@Version标注version乐观锁

## 三、前置依赖

### 3.1 前置任务

- P2-012-001-005-001 期初应收票Entity/DTO/VO定义（父任务）
...
| (按编写InitApInvoiceEntity类补充业务字段) | - | @TableField | - | - |
...
public class InitApInvoiceEntity extends BaseEntity {
    // 业务字段按编写InitApInvoiceEntity类定义
}
```
...
| 序号 | 文件路径 | 说明 |
|:---:|---------|------|
| 1 | src/main/java/com/erp/init/entity/InitApInvoiceEntity.java | Entity实体类 |
