# P2-012-001-005-001-002 编写InitApInvoiceCreateDTO

## 一、任务标识

| 属性 | 值 |
|------|-----|
| 任务编号 | P2-012-001-005-001-002 |
| 任务名称 | 编写InitApInvoiceCreateDTO |
| 所属模块 | P2-012 |
| 优先级 | P2 |
...
定义编写InitApInvoiceCreateDTO：CreateDTO使用@NotBlank/@NotNull/@DecimalMin等校验注解确保必填字段非空、数值字段范围合法；UpdateDTO继承CreateDTO并增加id字段；QueryDTO包含分页参数(pageNum/pageSize)+筛选条件字段+排序字段(sortField/sortOrder)
...
- P2-012-001-005-001-001 编写InitApInvoiceEntity类（前序兄弟任务）
...
| 字段名 | 类型 | 校验注解 | 说明 |
|--------|------|---------|------|
| (按编写InitApInvoiceCreateDTO列出必填字段) | - | @NotBlank/@NotNull | 必填字段 |
...
| 序号 | 文件路径 | 说明 |
|:---:|---------|------|
| 1 | src/main/java/com/erp/init/dto/InitApInvoiceCreateDTO.java | 新增DTO |
