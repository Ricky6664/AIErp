...
| 属性 | 值 |
|------|-----|
| 任务编号 | P2-011-002-012-002-001 |
| 任务名称 | 大屏布局容器开发（v-scale-screen组件+1920x1080/2K/4K自适应缩放+深色主题CSS+CSS clamp()响应式+网格布局Grid/Flex+组件拖拽配置位置） |
| 所属模块 | P2-011 |
| 优先级 | P2 |
| 任务类型 | 配置/基础设施 |
...
编写大屏布局容器开发（v-scale-screen组件+1920x1080/2K/4K自适应缩放+深色主题CSS+CSS clamp()响应式+网格布局Grid/Flex+组件拖拽配置位置）：配置项定义（Key命名规范report.feature.name）+默认值+环境差异（dev/test/prod）+配置说明注释，敏感配置加密存储
