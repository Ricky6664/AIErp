# P2-011-002-012-001-001 编写report_screen_config表DDL+Entity/Mapper/Service/Controller

## 一、任务标识

| 属性 | 值 |
|------|-----|
| 任务编号 | P2-011-002-012-001-001 |
| 任务名称 | 编写report_screen_config表DDL+Entity/Mapper/Service/Controller |
| 所属模块 | P2-011 |
| 优先级 | P2 |
| 任务类型 | DDL/数据库建表 |
...
编写report_screen_config表DDL+Entity/Mapper/Service/Controller的Flyway迁移脚本：定义完整字段（含10个通用必含字段id/tenant_id/created_at/updated_at/created_by/updated_by/is_deleted/owner_dept_id/owner_id/version）+ 业务字段 + 索引（含uk唯一索引WHERE is_deleted=false + idx查询索引），PostgreSQL 15+语法，脚本命名V{版本号}__create_{表名}.sql
...
| 11+ | (按编写report_screen_config表DDL+Entity/Mapper/Service/Controller补充业务字段) | - | - | - | - |
...
CREATE TABLE report (
    ...
    -- 业务字段按编写report_screen_config表DDL+Entity/Mapper/Service/Controller补充
);
