# P2-015-001-007-001-001-002 实现字典加载时按语言返回

## 一、任务标识

| 属性 | 值 |
|------|-----|
| 任务编号 | P2-015-001-007-001-001-002 |
| 任务名称 | 实现字典加载时按语言返回 |
| 所属模块 | P2-015 |
| 优先级 | P2 |
| 任务类型 | 综合开发任务 |
...
编写I18nService接口：声明page(QueryDTO)/getById/create/update/delete等业务方法；编写ServiceImpl实现类：含唯一性校验+状态流转校验+关联数据完整性校验+编码引擎调用+操作日志记录，@Transactional事务注解加在写操作方法

## 三、前置依赖

### 3.1 前置任务

- P2-015-001-007-001-001 实现字典项名称按语言翻译+字典加载时按当前语言返回字典项名称+缓存（父任务）
- P2-015-001-007-001-001-001 实现字典项名称按语言翻译（前序兄弟任务）
...
| 方法签名 | 功能 | 事务 |
|---------|------|------|
| PageResult<I18nListVO> page(I18nQueryDTO query) | 分页查询 | 无 |
| I18nDetailVO getById(Long id) | 详情查询 | 无 |
| Long create(I18nCreateDTO dto) | 新增 | @Transactional |
| void update(Long id, I18nUpdateDTO dto) | 修改 | @Transactional |
...
| 调用目标 | 调用方式 | 调用时机 |
|---------|---------|---------|
| 编码引擎 | codeGenerateService.generate("i18n_language") | 新增时 |
...
| 序号 | 文件路径 | 说明 |
|:---:|---------|------|
| 1 | src/main/java/com/erp/i18n/service/I18nService.java | Service接口 |
| 2 | src/main/java/com/erp/i18n/service/impl/I18nServiceImpl.java | Service实现类 |
