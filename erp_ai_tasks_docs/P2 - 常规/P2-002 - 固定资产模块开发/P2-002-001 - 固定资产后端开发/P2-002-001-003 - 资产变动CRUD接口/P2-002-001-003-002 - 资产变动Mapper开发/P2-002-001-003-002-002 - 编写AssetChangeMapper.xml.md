# P2-002-001-003-002-002 编写AssetChangeMapper.xml

## 一、任务标识

| 属性 | 值 |
|------|-----|
| 任务编号 | P2-002-001-003-002-002 |
| 任务名称 | 编写AssetChangeMapper.xml |
| 所属模块 | P2-002 |
| 优先级 | P2 |
| 任务类型 | Mapper数据访问层 |
...
编写AssetChangeMapper.xml：resultMap定义；自定义SQL(主从JOIN/动态条件<if>/<choose>)；分页使用MyBatis-Plus Page；#{}参数绑定防注入
...
- P2-002-001-003-002-001 编写AssetChangeMapper接口（前序兄弟任务）
...
<mapper namespace="com.xxx.asset.mapper.AssetChangeMapper">
    <resultMap id="BaseResultMap" type="com.xxx.asset.entity.AssetChangeEntity">
...
| 序号 | 文件路径 | 说明 |
|:---:|---------|------|
| 1 | src/main/resources/mapper/asset/AssetChangeMapper.xml | AssetChangeMapper XML |
