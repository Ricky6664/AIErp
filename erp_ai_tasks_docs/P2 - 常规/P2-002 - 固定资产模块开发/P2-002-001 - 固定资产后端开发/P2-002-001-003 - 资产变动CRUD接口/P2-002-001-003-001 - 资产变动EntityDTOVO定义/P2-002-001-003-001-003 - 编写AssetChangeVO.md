# P2-002-001-003-001-003 编写AssetChangeVO

## 一、任务标识

| 属性 | 值 |
|------|-----|
| 任务编号 | P2-002-001-003-001-003 |
| 任务名称 | 编写AssetChangeVO |
| 所属模块 | P2-002 |
| 优先级 | P2 |
...
定义AssetChangeVO详情对象：完整业务字段+关联翻译(deptName/userName)；@JsonFormat日期格式化；字典翻译字段(statusName)；BigDecimal金额精度
...
- P2-002-001-003-001-002 编写AssetChangeCreateDTO（前序兄弟任务）
...
public class AssetChangeVO {
    ...
    private List<AssetChangeDetailVO> details;
}
```
...
| 序号 | 文件路径 | 说明 |
|:---:|---------|------|
| 1 | src/main/java/com/erp/asset/vo/AssetChangeVO.java | AssetChangeVO |
