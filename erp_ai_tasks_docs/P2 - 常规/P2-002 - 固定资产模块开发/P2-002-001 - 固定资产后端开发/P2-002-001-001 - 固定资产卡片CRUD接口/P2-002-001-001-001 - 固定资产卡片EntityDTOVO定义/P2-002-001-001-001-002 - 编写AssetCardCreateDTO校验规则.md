...
| 属性 | 值 |
|------|-----|
| 任务编号 | P2-002-001-001-001-002 |
| 任务名称 | 编写AssetCardCreateDTO（@NotBlank资产编号+@NotBlank资产名称+@NotNull资产分类+@NotNull取得日期+@DecimalMin("0.01")原值>0+@DecimalMin("0")残值率0-100+@NotNull使用年限>0+@NotBlank折旧方法+@NotNull使用部门+校验规则：残值=原值×残值率+月折旧额根据折旧方法公式自动计算） |
| 所属模块 | P2-002 |
| 优先级 | P2 |
...
定义AssetCardCreateDTO：@NotBlank标注必填字符串(编号/名称)；@NotNull标注必填引用字段(部门ID/币种ID)；@Size限制字段长度；@DecimalMin金额≥0；嵌套明细列表@Valid+@NotEmpty级联校验；@DateTimeFormat日期格式
...
- P2-002-001-001-001-001 编写AssetEntity类（前序兄弟任务）
...
```java
@Data
public class AssetCardCreateDTO {
    ...
    private List<AssetCardDetailDTO> details;
...
| 序号 | 文件路径 | 说明 |
|:---:|---------|------|
| 1 | src/main/java/com/erp/asset/dto/AssetCardCreateDTO.java | AssetCard创建DTO |
