...
| 属性 | 值 |
|------|-----|
| 任务编号 | P2-002-001-004-001-001 |
| 任务名称 | 编写AssetDisposalEntity类（@TableName"fa_asset_disposal"+@TableId处置ID+处置编号disposal_no+处置日期disposal_date+处置方式disposal_type（出售/报废/捐赠/盘亏）+处置资产数量+原值合计+累计折旧合计+净值合计+处置收入disposal_income+处置费用disposal_expense+处置损益disposal_gain_loss+关联凭证ID voucher_id+状态status+制单人+制单日期+@TableLogic逻辑删除） |
| 所属模块 | P2-002 |
| 优先级 | P2 |
