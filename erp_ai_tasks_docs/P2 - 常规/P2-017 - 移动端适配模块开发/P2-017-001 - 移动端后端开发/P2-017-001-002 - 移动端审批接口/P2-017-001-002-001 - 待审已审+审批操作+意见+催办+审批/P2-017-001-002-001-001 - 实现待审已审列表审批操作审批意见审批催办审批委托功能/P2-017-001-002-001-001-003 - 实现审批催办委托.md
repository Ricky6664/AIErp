# P2-017-001-002-001-001-003 实现审批催办+委托

## 一、任务标识

| 属性 | 值 |
|------|-----|
| 任务编号 | P2-017-001-002-001-001-003 |
| 任务名称 | 实现审批催办+委托 |
| 所属模块 | P2-017 |
| 优先级 | P2 |
| 任务类型 | 综合开发任务 |
...
编写MobileService接口：声明page(QueryDTO)/getById/create/update/delete等业务方法；编写ServiceImpl实现类：含唯一性校验+状态流转校验+关联数据完整性校验+编码引擎调用+操作日志记录，@Transactional事务注解加在写操作方法

## 三、前置依赖

### 3.1 前置任务

- P2-017-001-002-001-001 实现待审/已审列表+审批操作+审批意见+审批催办+审批委托功能（父任务）
- P2-017-001-002-001-001-002 实现审批操作（前序兄弟任务）
...
| 方法签名 | 功能 | 事务 |
|---------|------|------|
| PageResult<MobileListVO> page(MobileQueryDTO query) | 分页查询 | 无 |
| MobileDetailVO getById(Long id) | 详情查询 | 无 |
| Long create(MobileCreateDTO dto) | 新增 | @Transactional |
| void update(Long id, MobileUpdateDTO dto) | 修改 | @Transactional |
...
| 调用目标 | 调用方式 | 调用时机 |
|---------|---------|---------|
| 编码引擎 | codeGenerateService.generate("mobile_menu") | 新增时 |
...
| 序号 | 文件路径 | 说明 |
|:---:|---------|------|
| 1 | src/main/java/com/erp/mobile/service/MobileService.java | Service接口 |
| 2 | src/main/java/com/erp/mobile/service/impl/MobileServiceImpl.java | Service实现类 |
