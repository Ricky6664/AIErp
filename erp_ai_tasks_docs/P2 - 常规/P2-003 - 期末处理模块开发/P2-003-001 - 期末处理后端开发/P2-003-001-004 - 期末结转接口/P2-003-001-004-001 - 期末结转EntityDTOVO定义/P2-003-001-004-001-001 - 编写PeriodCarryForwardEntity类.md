...
| 属性 | 值 |
|------|-----|
| 任务编号 | P2-003-001-004-001-001 |
| 任务名称 | 编写PeriodCarryForwardEntity类（@TableName"fin_period_carry_forward"+@TableId结转ID+结转编号carry_no+会计期间period_id+结转类型carry_type（损益结转/成本结转/增值税结转/所得税结转/利润分配结转）+结转金额carry_amount+转出科目from_account_id+转入科目to_account_id+关联凭证ID voucher_id+状态status+备注remark+@TableLogic逻辑删除） |
| 所属模块 | P2-003 |
| 优先级 | P2 |
...
定义PeriodCarryForwardEntity类：@TableName("period_period_carry_forward") + @TableId(type=IdType.ASSIGN_ID)雪花ID策略；业务字段使用@TableField显式映射；公共字段(create_by/create_time/update_by/update_time/tenant_id)通过MetaHandler自动填充；@TableLogic逻辑删除is_deleted默认0；@Version乐观锁version字段；implements Serializable

## 三、前置依赖

### 3.1 前置任务

- P2-003-001-004-001 期末结转Entity/DTO/VO定义（父任务）
...
```java
@Data
@TableName("period_period_carry_forward")
public class PeriodCarryForwardEntity implements Serializable {
...
| 序号 | 文件路径 | 说明 |
|:---:|---------|------|
| 1 | src/main/java/com/erp/period/entity/PeriodCarryForwardEntity.java | PeriodCarryForward实体类 |
...
| 序号 | 检查项 | 验证方法 |
|:---:|--------|---------|
| 1 | @TableName("period_period_carry_forward")正确 | 启动测试 |
