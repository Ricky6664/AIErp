# P2-003-001-004-001-003 编写CarryForwardVO

## 一、任务标识

| 属性 | 值 |
|------|-----|
| 任务编号 | P2-003-001-004-001-003 |
| 任务名称 | 编写CarryForwardVO |
| 所属模块 | P2-003 |
| 优先级 | P2 |
...
定义CarryForwardVO详情对象：完整业务字段+关联翻译(deptName/userName)；@JsonFormat日期格式化；字典翻译字段(statusName)；BigDecimal金额精度
...
- P2-003-001-004-001-002 编写CarryForwardExecuteDTO（@NotNull会计期间+@NotNull结转类型（多选）+试算标志preview_flag）（前序兄弟任务）
...
public class CarryForwardVO {
    ...
    private List<CarryForwardDetailVO> details;
}
```
...
| 序号 | 文件路径 | 说明 |
|:---:|---------|------|
| 1 | src/main/java/com/erp/period/vo/CarryForwardVO.java | CarryForwardVO |
