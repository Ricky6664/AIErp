# P2-003-001-004-002-001 编写CarryForwardService接口

## 一、任务标识

| 属性 | 值 |
|------|-----|
| 任务编号 | P2-003-001-004-002-001 |
| 任务名称 | 编写CarryForwardService接口 |
| 所属模块 | P2-003 |
| 优先级 | P2 |
| 任务类型 | Service服务层 |
...
定义CarryForwardService接口：extends IServiceX<CarryForwardEntity>；CRUD方法(create/update/delete/getDetail/page)+业务方法(submit/checkBudget等)
...
```java
public interface CarryForwardService extends IServiceX<CarryForwardEntity> {
    CarryForwardVO create(CarryForwardCreateDTO dto);
    Boolean update(CarryForwardUpdateDTO dto);
    Boolean delete(Long id);
    CarryForwardVO getDetail(Long id);
    Page<CarryForwardListVO> getPage(CarryForwardQueryDTO query);
...
| 序号 | 文件路径 | 说明 |
|:---:|---------|------|
| 1 | src/main/java/com/erp/period/service/CarryForwardService.java | CarryForwardService接口 |
