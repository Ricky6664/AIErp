# P2-003-001-003-001-004 编写ExchangeGainLossVO

## 一、任务标识

| 属性 | 值 |
|------|-----|
| 任务编号 | P2-003-001-003-001-004 |
| 任务名称 | 编写ExchangeGainLossVO |
| 所属模块 | P2-003 |
| 优先级 | P2 |
...
定义ExchangeGainLossVO详情对象：完整业务字段+关联翻译(deptName/userName)；@JsonFormat日期格式化；字典翻译字段(statusName)；BigDecimal金额精度
...
- P2-003-001-003-001-003 编写ExchangeGainLossExecuteDTO（@NotNull会计期间+@NotNull期末汇率+币种可选（默认所有外币科目）+调汇日期+试算标志preview_flag）（前序兄弟任务）
...
```java
@Data
public class ExchangeGainLossVO {
    private Long id;
    private String code;
    private String name;
    ...
    private List<ExchangeGainLossDetailVO> details;
}
```
...
| 序号 | 文件路径 | 说明 |
|:---:|---------|------|
| 1 | src/main/java/com/erp/period/vo/ExchangeGainLossVO.java | ExchangeGainLossVO |
