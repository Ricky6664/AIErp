# P2-003-001-003-001-002 编写ExchangeGainLossDetailEntity类

## 一、任务标识

| 属性 | 值 |
|------|-----|
| 任务编号 | P2-003-001-003-001-002 |
| 任务名称 | 编写ExchangeGainLossDetailEntity类 |
| 所属模块 | P2-003 |
| 优先级 | P2 |
...
定义ExchangeGainLossEntity类：@TableName("period_exchange_gain_loss") + @TableId(type=IdType.ASSIGN_ID)雪花ID策略；业务字段使用@TableField显式映射；公共字段(create_by/create_time/update_by/update_time/tenant_id)通过MetaHandler自动填充；@TableLogic逻辑删除is_deleted默认0；@Version乐观锁version字段；implements Serializable

## 三、前置依赖

### 3.1 前置任务

- P2-003-001-003-001 期末调汇Entity/DTO/VO定义（父任务）
- P2-003-001-003-001-001 编写ExchangeGainLossEntity类（前序兄弟任务）
...
```java
@Data
@TableName("period_exchange_gain_loss")
public class ExchangeGainLossEntity implements Serializable {
...
| 序号 | 文件路径 | 说明 |
|:---:|---------|------|
| 1 | src/main/java/com/erp/period/entity/ExchangeGainLossEntity.java | ExchangeGainLoss实体类 |
...
| 序号 | 检查项 | 验证方法 |
|:---:|--------|---------|
| 1 | @TableName("period_exchange_gain_loss")正确 | 启动测试 |
