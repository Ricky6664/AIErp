# P2-003-001-001-003-001 编写PeriodCloseService接口

## 一、任务标识

| 属性 | 值 |
|------|-----|
| 任务编号 | P2-003-001-001-003-001 |
| 任务名称 | 编写PeriodCloseService接口 |
| 所属模块 | P2-003 |
| 优先级 | P2 |
| 任务类型 | Service服务层 |
...
定义PeriodCloseService接口：extends IServiceX<PeriodCloseEntity>；CRUD方法(create/update/delete/getDetail/page)+业务方法(submit/checkBudget等)
...
```java
public interface PeriodCloseService extends IServiceX<PeriodCloseEntity> {
    PeriodCloseVO create(PeriodCloseCreateDTO dto);
    Boolean update(PeriodCloseUpdateDTO dto);
    Boolean delete(Long id);
    PeriodCloseVO getDetail(Long id);
    Page<PeriodCloseListVO> getPage(PeriodCloseQueryDTO query);
...
| 序号 | 文件路径 | 说明 |
|:---:|---------|------|
| 1 | src/main/java/com/erp/period/service/PeriodCloseService.java | PeriodCloseService接口 |
