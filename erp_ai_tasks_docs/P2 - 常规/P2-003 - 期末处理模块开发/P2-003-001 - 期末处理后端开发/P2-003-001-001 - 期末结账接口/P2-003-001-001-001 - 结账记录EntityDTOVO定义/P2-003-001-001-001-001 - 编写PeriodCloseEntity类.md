# P2-003-001-001-001-001 编写PeriodCloseEntity类

## 一、任务标识

| 属性 | 值 |
|------|-----|
| 任务编号 | P2-003-001-001-001-001 |
| 任务名称 | 编写PeriodCloseEntity类 |
| 所属模块 | P2-003 |
| 优先级 | P2 |
...
定义PeriodCloseEntity类：@TableName("period_period_close") + @TableId(type=IdType.ASSIGN_ID)雪花ID策略；业务字段使用@TableField显式映射；公共字段(create_by/create_time/update_by/update_time/tenant_id)通过MetaHandler自动填充；@TableLogic逻辑删除is_deleted默认0；@Version乐观锁version字段；implements Serializable

## 三、前置依赖

### 3.1 前置任务

- P2-003-001-001-001 结账记录Entity/DTO/VO定义（父任务）
...
```java
@Data
@TableName("period_period_close")
public class PeriodCloseEntity implements Serializable {
...
| 序号 | 文件路径 | 说明 |
|:---:|---------|------|
| 1 | src/main/java/com/erp/period/entity/PeriodCloseEntity.java | PeriodClose实体类 |
...
| 序号 | 检查项 | 验证方法 |
|:---:|--------|---------|
| 1 | @TableName("period_period_close")正确 | 启动测试 |
