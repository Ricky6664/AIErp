# P2-003-001-001-001-004 编写PeriodCloseResultVO

## 一、任务标识

| 属性 | 值 |
|------|-----|
| 任务编号 | P2-003-001-001-001-004 |
| 任务名称 | 编写PeriodCloseResultVO |
| 所属模块 | P2-003 |
| 优先级 | P2 |
...
定义PeriodCloseResultVO详情对象：完整业务字段+关联翻译(deptName/userName)；@JsonFormat日期格式化；字典翻译字段(statusName)；BigDecimal金额精度
...
- P2-003-001-001-001-003 编写PeriodCloseCheckResultVO（前序兄弟任务）
...
```java
@Data
public class PeriodCloseResultVO {
    private Long id;
    private String code;
    private String name;
    ...
    private List<PeriodCloseResultDetailVO> details;
}
```
...
| 序号 | 文件路径 | 说明 |
|:---:|---------|------|
| 1 | src/main/java/com/erp/period/vo/PeriodCloseResultVO.java | PeriodCloseResultVO |
