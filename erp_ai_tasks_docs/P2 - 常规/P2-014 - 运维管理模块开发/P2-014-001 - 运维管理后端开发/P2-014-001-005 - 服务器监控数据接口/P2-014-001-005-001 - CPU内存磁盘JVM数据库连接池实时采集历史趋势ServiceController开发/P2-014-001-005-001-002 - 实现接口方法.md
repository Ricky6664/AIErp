# P2-014-001-005-001-002 实现接口方法

## 一、任务标识

| 属性 | 值 |
|------|-----|
| 任务编号 | P2-014-001-005-001-002 |
| 任务名称 | 实现接口方法 |
| 所属模块 | P2-014 |
| 优先级 | P2 |
| 任务类型 | 综合开发任务 |
...
实现OpsController接口方法：@RequestBody接收DTO+@Valid触发校验+调用Service+Result<T>包装响应，@SaCheckPermission权限注解，@OperLog操作日志注解，异常由GlobalExceptionHandler统一处理
...
- P2-014-001-005-001-001 定义Controller类+@RequestMapping路径+注入Service（前序兄弟任务）
...
```java
@PostMapping("/create")
@Operation(summary = "新增运维管理")
@SaCheckPermission("ops:add")
@OperLog(module = "运维管理", action = "新增")
