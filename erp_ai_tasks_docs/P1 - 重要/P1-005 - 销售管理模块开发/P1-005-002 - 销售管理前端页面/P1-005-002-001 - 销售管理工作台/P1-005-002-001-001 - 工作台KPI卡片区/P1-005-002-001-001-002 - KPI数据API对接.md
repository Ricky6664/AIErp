# P1-005-002-001-001-002 KPI数据API对接

## 一、任务标识

| 属性 | 值 |
|------|-----|
| 任务编号 | P1-005-002-001-001-002 |
| 任务名称 | KPI数据API对接 |
| 所属模块 | P1-005 |
| 优先级 | P1 |
| 任务类型 | 综合开发任务 |
...
对接SaleQuotation工作台KPI数据API：调用GET /api//workbench接口、响应数据绑定KPI卡片、异常降级显示
...
- P1-005-002-001-001 工作台KPI卡片区（父任务）
- P1-005-002-001-001-001 KPI卡片组件开发（前序兄弟任务）
...
const loadKpiData = async () => {
  loading.value = true
  try {
    const res = await api.get("/api/sale/workbench")
...
| 序号 | 文件路径 | 说明 |
|:---:|---------|------|
| 1 | src/api/saleWorkbench.ts | 工作台API |
| 2 | src/views/sale/workbench/index.vue | 工作台页面 |
