...
| 属性 | 值 |
|------|-----|
| 任务编号 | P1-005-001-003-003-001 |
| 任务名称 | 定义Controller类+@RequestMapping路径 |
| 所属模块 | P1-005 |
| 优先级 | P1 |
| 任务类型 | Controller接口层 |
...
定义SaleOrderChangeController类，@RestController+@RequestMapping("/api/sale/order-change")，注入ISaleOrderChangeService，声明GET/POST/PUT/DELETE端点方法
...
```java
@RestController
@RequestMapping("/api/sale/order-change")
@RequiredArgsConstructor
@Tag(name = "SaleQuotation管理")
public class SaleOrderChangeController {
    private final ISaleOrderChangeService saleService;
...
| 序号 | 文件路径 | 说明 |
|:---:|---------|------|
| 1 | src/main/java/com/erp/sale/controller/SaleOrderChangeController.java | Controller类 |

## 七、验收标准

| 序号 | 检查项 | 验证方法 |
|:---:|--------|---------|
| 1 | SaleOrderChangeController的@RestController+@RequestMapping("/api/sale/order-change")正确 | 代码检查 |
