# P1-006-002-001-002-002 图表数据API对接

## 一、任务标识

| 属性 | 值 |
|------|-----|
| 任务编号 | P1-006-002-001-002-002 |
| 任务名称 | 图表数据API对接 |
| 所属模块 | P1-006 |
| 优先级 | P1 |
| 任务类型 | 前端图表组件 |
...
对接PurchaseOrder工作台图表数据API：调用统计接口、数据转换为ECharts series格式、加载状态+异常处理
...
- P1-006-002-001-002 工作台图表区（父任务）
- P1-006-002-001-002-001 ECharts图表组件开发（前序兄弟任务）
...
const loadChartData = async () => {
  const res = await api.get("/api/purchase/statistics")
  chartOption.value = {
...
| 序号 | 文件路径 | 说明 |
|:---:|---------|------|
| 1 | src/api/purchaseStatistics.ts | 统计API |
| 2 | src/views/purchase/workbench/index.vue | 图表数据绑定 |
