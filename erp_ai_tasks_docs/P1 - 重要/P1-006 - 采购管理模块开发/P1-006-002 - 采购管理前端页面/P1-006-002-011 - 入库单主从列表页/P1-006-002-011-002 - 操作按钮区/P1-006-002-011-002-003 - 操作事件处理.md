# P1-006-002-011-002-003 操作事件处理

## 一、任务标识

| 属性 | 值 |
|------|-----|
| 任务编号 | P1-006-002-011-002-003 |
| 任务名称 | 操作事件处理 |
| 所属模块 | P1-006 |
| 优先级 | P1 |
| 任务类型 | 综合开发任务 |
...
实现PurchaseOrder操作事件处理：新增(打开表单)、编辑(回填数据)、删除(确认弹窗+调用DELETE接口)、刷新(重新加载列表)
...
- P1-006-002-011-002 操作按钮区（父任务）
- P1-006-002-011-002-002 批量操作逻辑（前序兄弟任务）
...
### 5.1 操作事件方法
```typescript
const handleAdd = () => { router.push({ name: 'PurchaseOrderForm' }) }

const handleEdit = () => {
  const row = selectedRows.value[0]
  router.push({ name: 'PurchaseOrderForm', params: { id: row.id } })
}

const handleDelete = async () => {
  await ElMessageBox.confirm(确认删除选中的  条记录？, '提示', { type: 'warning' })
  const ids = selectedRows.value.map(r => r.id)
  await api.batchDeletePurchaseOrder(ids)
...
| 序号 | 文件路径 | 说明 |
|:---:|---------|------|
| 1 | src/views/purchase/index.vue | 操作事件方法 |
