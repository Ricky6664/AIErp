# P1-006-001-005-002-001 定义Service接口

## 一、任务标识

| 属性 | 值 |
|------|-----|
| 任务编号 | P1-006-001-005-002-001 |
| 任务名称 | 定义Service接口 |
| 所属模块 | P1-006 |
| 优先级 | P1 |
| 任务类型 | Service服务层 |
...
定义IPurchaseOrderService接口，继承IServiceX<PurchaseOrderEntity>，声明pur_inquiry表CRUD方法+业务方法（提交审核、反审、下推等）
...
### 5.1 Service接口定义
```java
public interface IPurchaseOrderService extends IServiceX<PurchaseOrderEntity> {
    /** 分页查询列表 */
    Page<PurchaseOrderListVO> pageList(PurchaseOrderQueryDTO query);
    ...
    /** 修改 */
    Boolean update(PurchaseOrderUpdateDTO dto);
...
| 序号 | 检查项 | 验证方法 |
|:---:|--------|---------|
| 1 | IPurchaseOrderService继承IServiceX<PurchaseOrderEntity> | 代码检查 |
