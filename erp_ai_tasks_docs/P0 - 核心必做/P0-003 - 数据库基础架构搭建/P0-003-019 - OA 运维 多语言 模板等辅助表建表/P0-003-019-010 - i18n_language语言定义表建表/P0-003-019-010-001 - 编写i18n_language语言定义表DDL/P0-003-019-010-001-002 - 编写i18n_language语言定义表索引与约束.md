# P0-003-019-010-001-002 编写i18n_language语言定义表索引与约束

## 一、任务标识

| 属性 | 值 |
|------|-----|
| 任务编号 | P0-003-019-010-001-002 |
| 任务名称 | 编写i18n_language语言定义表索引与约束 |
| 所属模块 | P0-003 |
| 优先级 | P0 |
| 任务类型 | 综合开发任务 |
...
- P0-003-019-010-001-001 编写CREATE TABLE语句（前序兄弟任务）
...
### 5.1 主键约束
- `ALTER TABLE n_language ADD CONSTRAINT pk_n_language PRIMARY KEY (id)`

### 5.2 部分唯一索引（关键：必须含WHERE is_deleted=false）
- `CREATE UNIQUE INDEX uk_n_language_code ON n_language(code) WHERE is_deleted = false`
- 避免boolean类型is_deleted在联合唯一索引中的"一删一活"陷阱

### 5.3 多租户联合索引（tenant_id必须为首列）
- `CREATE INDEX idx_n_language_tenant_code ON n_language(tenant_id, code)`
- `CREATE INDEX idx_n_language_tenant_status ON n_language(tenant_id, status)`

### 5.4 业务查询索引
- 外键关联字段索引、常用查询字段索引（状态、日期）、树形结构索引（parent_id）

### 5.5 Flyway迁移脚本
- 命名：`V20260526001__create_n_language_indexes.sql`
...
| 序号 | 文件路径 | 说明 |
|:---:|---------|------|
| 1 | db/migration/V20260526001__create_n_language_indexes.sql | n_language表索引与约束脚本 |
| 2 | db/migration/V20260526001__drop_n_language_indexes.sql | 索引回滚脚本(DROP INDEX) |
...
| 序号 | 检查项 | 验证方法 |
|:---:|--------|--------|
| 1 | 主键约束pk_n_language创建成功 | information_schema.table_constraints查询 |
