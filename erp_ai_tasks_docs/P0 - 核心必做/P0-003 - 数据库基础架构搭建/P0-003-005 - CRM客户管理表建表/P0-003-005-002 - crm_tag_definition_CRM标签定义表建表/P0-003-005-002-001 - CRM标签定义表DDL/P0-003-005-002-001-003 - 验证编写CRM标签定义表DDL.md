# P0-003-005-002-001-003 验证编写CRM标签定义表DDL
...
- P0-003-005-002-001-002 编写CRM标签定义表索引与约束（前序兄弟任务）
...
### 5.1 DDL执行验证
- 执行`SELECT tablename FROM pg_tables WHERE schemaname='public' AND tablename='xxx'`确认表已创建
- 执行`SELECT column_name, data_type, is_nullable FROM information_schema.columns WHERE table_name='xxx'`验证字段定义
- 执行`SELECT indexname, indexdef FROM pg_indexes WHERE tablename='xxx'`验证索引
...
| 序号 | 文件路径 | 说明 |
|:---:|---------|------|
| 1 | db/migration/V20260526001__task_P0_003_005_002_001_003.sql | Flyway迁移脚本 |
| 2 | docs/specs/P0_003_005_002_001_003_spec.md | 任务规范/验证文档 |
