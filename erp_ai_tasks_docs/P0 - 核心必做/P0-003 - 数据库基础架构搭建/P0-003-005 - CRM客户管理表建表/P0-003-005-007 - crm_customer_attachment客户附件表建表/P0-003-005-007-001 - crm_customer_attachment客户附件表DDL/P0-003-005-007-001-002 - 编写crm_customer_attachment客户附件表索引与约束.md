# P0-003-005-007-001-002 编写crm_customer_attachment客户附件表索引与约束

## 一、任务标识

| 属性 | 值 |
|------|-----|
| 任务编号 | P0-003-005-007-001-002 |
| 任务名称 | 编写crm_customer_attachment客户附件表索引与约束 |
| 所属模块 | P0-003 |
| 优先级 | P0 |
| 任务类型 | 综合开发任务 |
...
- P0-003-005-007-001-001 编写CREATE TABLE语句（前序兄弟任务）
...
### 5.1 主键约束
- `ALTER TABLE crm_customer_attachment ADD CONSTRAINT pk_crm_customer_attachment PRIMARY KEY (id)`
...
### 5.5 Flyway迁移脚本
- 命名：`V20260526001__create_crm_customer_attachment_indexes.sql`
...
| 序号 | 文件路径 | 说明 |
|:---:|---------|------|
| 1 | db/migration/V20260526001__create_crm_customer_attachment_indexes.sql | crm_customer_attachment表索引与约束脚本 |
| 2 | db/migration/V20260526001__drop_crm_customer_attachment_indexes.sql | 索引回滚脚本(DROP INDEX) |
...
| 序号 | 检查项 | 验证方法 |
|:---:|--------|--------|
| 1 | 主键约束pk_crm_customer_attachment创建成功 | information_schema.table_constraints查询 |
