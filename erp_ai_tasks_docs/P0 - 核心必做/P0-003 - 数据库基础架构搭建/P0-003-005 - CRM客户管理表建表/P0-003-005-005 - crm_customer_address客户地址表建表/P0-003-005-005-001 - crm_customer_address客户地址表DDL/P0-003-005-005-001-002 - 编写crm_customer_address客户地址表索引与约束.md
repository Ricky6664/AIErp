# P0-003-005-005-001-002 编写crm_customer_address客户地址表索引与约束

## 一、任务标识

| 属性 | 值 |
|------|-----|
| 任务编号 | P0-003-005-005-001-002 |
| 任务名称 | 编写crm_customer_address客户地址表索引与约束 |
| 所属模块 | P0-003 |
| 优先级 | P0 |
| 任务类型 | 综合开发任务 |
...
为crm_customer_address(客户地址表)创建索引与约束：主键PK(id)、部分唯一索引UK(含WHERE is_deleted=false)、tenant_id首列联合索引、业务查询B-Tree索引，确保索引命名规范(uk_/idx_前缀)

## 三、前置依赖

### 3.1 前置任务

- P0-003-005-005-001 编写crm_customer_address客户地址表DDL（父任务）
- P0-003-005-005-001-001 编写CREATE TABLE语句（前序兄弟任务）
...
### 5.1 主键约束
- `ALTER TABLE crm_customer_address ADD CONSTRAINT pk_crm_customer_address PRIMARY KEY (id)`

### 5.2 部分唯一索引（关键：必须含WHERE is_deleted=false）
- `CREATE UNIQUE INDEX uk_crm_customer_address_code ON crm_customer_address(code) WHERE is_deleted = false`
- 避免boolean类型is_deleted在联合唯一索引中的"一删一活"陷阱

### 5.3 多租户联合索引（tenant_id必须为首列）
- `CREATE INDEX idx_crm_customer_address_tenant_code ON crm_customer_address(tenant_id, code)`
...
### 5.5 Flyway迁移脚本
- 命名：`V20260526001__create_crm_customer_address_indexes.sql`
...
| 序号 | 文件路径 | 说明 |
|:---:|---------|------|
| 1 | db/migration/V20260526001__create_crm_customer_address_indexes.sql | crm_customer_address表索引与约束脚本 |
| 2 | db/migration/V20260526001__drop_crm_customer_address_indexes.sql | 索引回滚脚本(DROP INDEX) |
...
| 序号 | 检查项 | 验证方法 |
|:---:|--------|--------|
| 1 | 主键约束pk_crm_customer_address创建成功 | information_schema.table_constraints查询 |
