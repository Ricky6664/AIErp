# P0-003-007-007-001-002 编写inv_stocktaking_detail盘点主从表索引与约束

## 一、任务标识

| 属性 | 值 |
|------|-----|
| 任务编号 | P0-003-007-007-001-002 |
| 任务名称 | 编写inv_stocktaking_detail盘点主从表索引与约束 |
| 所属模块 | P0-003 |
| 优先级 | P0 |
| 任务类型 | 综合开发任务 |
...
- P0-003-007-007-001 编写inv_stocktaking_detail盘点主从表DDL（父任务）
- P0-003-007-007-001-001 编写CREATE TABLE语句（前序兄弟任务）
...
### 5.1 主键约束
- `ALTER TABLE inv_stocktaking_detail ADD CONSTRAINT pk_inv_stocktaking_detail PRIMARY KEY (id)`
...
- `CREATE INDEX idx_inv_stocktaking_detail_tenant_status ON inv_stocktaking_detail(tenant_id, status)`

### 5.4 业务查询索引
- 外键关联字段索引、常用查询字段索引（状态、日期）、树形结构索引（parent_id）

### 5.5 Flyway迁移脚本
- 命名：`V20260526001__create_inv_stocktaking_detail_indexes.sql`
...
| 序号 | 文件路径 | 说明 |
|:---:|---------|------|
| 1 | db/migration/V20260526001__create_inv_stocktaking_detail_indexes.sql | inv_stocktaking_detail表索引与约束脚本 |
| 2 | db/migration/V20260526001__drop_inv_stocktaking_detail_indexes.sql | 索引回滚脚本(DROP INDEX) |
...
| 序号 | 检查项 | 验证方法 |
|:---:|--------|--------|
| 1 | 主键约束pk_inv_stocktaking_detail创建成功 | information_schema.table_constraints查询 |
