# P0-003-007-002-001-003 验证编写inv_location库位管理表DDL
...
验证inv_location(库位管理表)的DDL脚本：确认CREATE TABLE执行成功、字段类型/长度/约束与设计文档一致、索引创建成功、Flyway版本号无冲突、COMMENT注释完整
...
- P0-003-007-002-001-002 编写inv_location库位管理表索引与约束（前序兄弟任务）
...
### 5.1 DDL执行验证
- 执行`SELECT tablename FROM pg_tables WHERE schemaname='public' AND tablename='inv_location'`确认表已创建
- 执行`SELECT column_name, data_type, is_nullable FROM information_schema.columns WHERE table_name='inv_location'`验证字段定义
- 执行`SELECT indexname, indexdef FROM pg_indexes WHERE tablename='inv_location'`验证索引
...
| 序号 | 文件路径 | 说明 |
|:---:|---------|------|
| 1 | db/migration/V20260526001__verify_inv_location.sql | inv_location表DDL验证查询脚本 |
| 2 | docs/verification/inv_location_verify_report.md | 验证报告记录 |
