# P0-003-007-003-001-002 编写inv_stock库存实时表索引与约束

## 一、任务标识

| 属性 | 值 |
|------|-----|
| 任务编号 | P0-003-007-003-001-002 |
| 任务名称 | 编写inv_stock库存实时表索引与约束 |
| 所属模块 | P0-003 |
| 优先级 | P0 |
| 任务类型 | 综合开发任务 |
...
为inv_stock(库存实时表)创建索引与约束：主键PK(id)、部分唯一索引UK(含WHERE is_deleted=false)、tenant_id首列联合索引、业务查询B-Tree索引，确保索引命名规范(uk_/idx_前缀)
...
- P0-003-007-003-001-001 编写CREATE TABLE语句（前序兄弟任务）
...
### 5.1 主键约束
- `ALTER TABLE inv_stock ADD CONSTRAINT pk_inv_stock PRIMARY KEY (id)`

### 5.2 部分唯一索引（关键：必须含WHERE is_deleted=false）
- `CREATE UNIQUE INDEX uk_inv_stock_code ON inv_stock(code) WHERE is_deleted = false`
- 避免boolean类型is_deleted在联合唯一索引中的"一删一活"陷阱

### 5.3 多租户联合索引（tenant_id必须为首列）
- `CREATE INDEX idx_inv_stock_tenant_code ON inv_stock(tenant_id, code)`
...
### 5.5 Flyway迁移脚本
- 命名：`V20260526001__create_inv_stock_indexes.sql`
...
| 序号 | 文件路径 | 说明 |
|:---:|---------|------|
| 1 | db/migration/V20260526001__create_inv_stock_indexes.sql | inv_stock表索引与约束脚本 |
| 2 | db/migration/V20260526001__drop_inv_stock_indexes.sql | 索引回滚脚本(DROP INDEX) |
...
| 序号 | 检查项 | 验证方法 |
|:---:|--------|--------|
| 1 | 主键约束pk_inv_stock创建成功 | information_schema.table_constraints查询 |
