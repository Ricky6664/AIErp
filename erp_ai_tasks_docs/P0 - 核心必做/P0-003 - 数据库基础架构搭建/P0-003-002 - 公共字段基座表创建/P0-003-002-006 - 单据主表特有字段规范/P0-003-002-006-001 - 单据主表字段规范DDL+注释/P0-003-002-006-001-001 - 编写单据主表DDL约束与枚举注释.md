...
完成编写DDL+约束（bill_no VARCHAR(32) NOT NULL UNIQUE含is_deleted+bill_status SMALLINT DEFAULT 0+枚举注释）的开发工作，编写PostgreSQL 15+ DDL/DML脚本，遵循Flyway迁移命名规范V{yyyyMMdd}{seq}__{description}.sql，确保通用字段10个完整、部分唯一索引包含WHERE is_deleted=false

## 三、前置依赖

### 3.1 前置任务

- P0-003-002-006-001 编写单据主表字段规范DDL+注释（父任务）
...
### 5.1 核心功能
1. 实现编写DDL+约束（bill_no VARCHAR(32) NOT NULL UNIQUE含is_deleted+bill_status SMALLINT DEFAULT 0+枚举注释）的核心逻辑
...
| 序号 | 文件路径 | 说明 |
|:---:|---------|------|
| 1 | db/migration/V20260526001__task_P0_003_002_006_001_001.sql | Flyway迁移脚本 |
| 2 | docs/specs/P0_003_002_006_001_001_spec.md | 任务规范/验证文档 |
