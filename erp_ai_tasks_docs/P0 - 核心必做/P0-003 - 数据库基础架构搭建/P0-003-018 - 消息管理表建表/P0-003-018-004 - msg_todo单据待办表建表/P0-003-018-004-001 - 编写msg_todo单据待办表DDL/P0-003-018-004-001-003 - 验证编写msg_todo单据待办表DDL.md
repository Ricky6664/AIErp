# P0-003-018-004-001-003 验证编写msg_todo单据待办表DDL

## 一、任务标识

| 属性 | 值 |
|------|-----|
| 任务编号 | P0-003-018-004-001-003 |
| 任务名称 | 验证编写msg_todo单据待办表DDL |
| 所属模块 | P0-003 |
| 优先级 | P0 |
| 任务类型 | DDL/数据库建表 |
...
验证msg_todo(单据待办表)的DDL脚本：确认CREATE TABLE执行成功、字段类型/长度/约束与设计文档一致、索引创建成功、Flyway版本号无冲突、COMMENT注释完整
...
- P0-003-018-004-001-002 编写msg_todo单据待办表索引与约束（前序兄弟任务）
...
### 5.1 DDL执行验证
- 执行`SELECT tablename FROM pg_tables WHERE schemaname='public' AND tablename='msg_todo'`确认表已创建
- 执行`SELECT column_name, data_type, is_nullable FROM information_schema.columns WHERE table_name='msg_todo'`验证字段定义
- 执行`SELECT indexname, indexdef FROM pg_indexes WHERE tablename='msg_todo'`验证索引
...
| 序号 | 文件路径 | 说明 |
|:---:|---------|------|
| 1 | db/migration/V20260526001__verify_msg_todo.sql | msg_todo表DDL验证查询脚本 |
| 2 | docs/verification/msg_todo_verify_report.md | 验证报告记录 |
