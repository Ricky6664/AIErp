# P0-003-016-003-001-002 编写fa_asset固定资产卡片表索引与约束

## 一、任务标识

| 属性 | 值 |
|------|-----|
| 任务编号 | P0-003-016-003-001-002 |
| 任务名称 | 编写fa_asset固定资产卡片表索引与约束 |
| 所属模块 | P0-003 |
| 优先级 | P0 |
| 任务类型 | 综合开发任务 |
...
- P0-003-016-003-001-001 编写CREATE TABLE语句（前序兄弟任务）
...
### 5.1 主键约束
- `ALTER TABLE fa_asset ADD CONSTRAINT pk_fa_asset PRIMARY KEY (id)`

### 5.2 部分唯一索引（关键：必须含WHERE is_deleted=false）
- `CREATE UNIQUE INDEX uk_fa_asset_code ON fa_asset(code) WHERE is_deleted = false`
- 避免boolean类型is_deleted在联合唯一索引中的"一删一活"陷阱

### 5.3 多租户联合索引（tenant_id必须为首列）
- `CREATE INDEX idx_fa_asset_tenant_code ON fa_asset(tenant_id, code)`
- `CREATE INDEX idx_fa_asset_tenant_status ON fa_asset(tenant_id, status)`

### 5.4 业务查询索引
- 外键关联字段索引、常用查询字段索引（状态、日期）、树形结构索引（parent_id）

### 5.5 Flyway迁移脚本
- 命名：`V20260526001__create_fa_asset_indexes.sql`
...
| 序号 | 文件路径 | 说明 |
|:---:|---------|------|
| 1 | db/migration/V20260526001__create_fa_asset_indexes.sql | fa_asset表索引与约束脚本 |
| 2 | db/migration/V20260526001__drop_fa_asset_indexes.sql | 索引回滚脚本(DROP INDEX) |
...
| 序号 | 检查项 | 验证方法 |
|:---:|--------|--------|
| 1 | 主键约束pk_fa_asset创建成功 | information_schema.table_constraints查询 |
