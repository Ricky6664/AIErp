# P0-003-014-008-001-002 编写ap_invoice_detail进项发票主从表索引与约束

## 一、任务标识

| 属性 | 值 |
|------|-----|
| 任务编号 | P0-003-014-008-001-002 |
| 任务名称 | 编写ap_invoice_detail进项发票主从表索引与约束 |
| 所属模块 | P0-003 |
| 优先级 | P0 |
...
- P0-003-014-008-001-001 编写CREATE TABLE语句（前序兄弟任务）
...
### 5.1 主键约束
- `ALTER TABLE ap_invoice_detail ADD CONSTRAINT pk_ap_invoice_detail PRIMARY KEY (id)`

### 5.2 部分唯一索引（关键：必须含WHERE is_deleted=false）
- `CREATE UNIQUE INDEX uk_ap_invoice_detail_code ON ap_invoice_detail(code) WHERE is_deleted = false`
- 避免boolean类型is_deleted在联合唯一索引中的"一删一活"陷阱

### 5.3 多租户联合索引（tenant_id必须为首列）
- `CREATE INDEX idx_ap_invoice_detail_tenant_code ON ap_invoice_detail(tenant_id, code)`
- `CREATE INDEX idx_ap_invoice_detail_tenant_status ON ap_invoice_detail(tenant_id, status)`

### 5.4 业务查询索引
- 外键关联字段索引、常用查询字段索引（状态、日期）、树形结构索引（parent_id）

### 5.5 Flyway迁移脚本
- 命名：`V20260526001__create_ap_invoice_detail_indexes.sql`
...
| 序号 | 文件路径 | 说明 |
|:---:|---------|------|
| 1 | db/migration/V20260526001__create_ap_invoice_detail_indexes.sql | ap_invoice_detail表索引与约束脚本 |
| 2 | db/migration/V20260526001__drop_ap_invoice_detail_indexes.sql | 索引回滚脚本(DROP INDEX) |
...
| 序号 | 检查项 | 验证方法 |
|:---:|--------|--------|
| 1 | 主键约束pk_ap_invoice_detail创建成功 | information_schema.table_constraints查询 |
