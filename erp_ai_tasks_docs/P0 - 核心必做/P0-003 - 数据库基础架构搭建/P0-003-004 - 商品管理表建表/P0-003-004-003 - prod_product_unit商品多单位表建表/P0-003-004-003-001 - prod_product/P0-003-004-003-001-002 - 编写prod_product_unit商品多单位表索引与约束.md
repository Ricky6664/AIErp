# P0-003-004-003-001-002 编写prod_product_unit商品多单位表索引与约束

## 一、任务标识

| 属性 | 值 |
|------|-----|
| 任务编号 | P0-003-004-003-001-002 |
| 任务名称 | 编写prod_product_unit商品多单位表索引与约束 |
| 所属模块 | P0-003 |
| 优先级 | P0 |
| 任务类型 | 综合开发任务 |
...
为prod_product_unit(商品多单位表)创建索引与约束：主键PK(id)、部分唯一索引UK(含WHERE is_deleted=false)、tenant_id首列联合索引、业务查询B-Tree索引，确保索引命名规范(uk_/idx_前缀)
...
- P0-003-004-003-001-001 编写CREATE TABLE语句（前序兄弟任务）
...
### 5.1 主键约束
- `ALTER TABLE prod_product_unit ADD CONSTRAINT pk_prod_product_unit PRIMARY KEY (id)`

### 5.2 部分唯一索引（关键：必须含WHERE is_deleted=false）
- `CREATE UNIQUE INDEX uk_prod_product_unit_code ON prod_product_unit(code) WHERE is_deleted = false`
- 避免boolean类型is_deleted在联合唯一索引中的"一删一活"陷阱

### 5.3 多租户联合索引（tenant_id必须为首列）
- `CREATE INDEX idx_prod_product_unit_tenant_code ON prod_product_unit(tenant_id, code)`
- `CREATE INDEX idx_prod_product_unit_tenant_status ON prod_product_unit(tenant_id, status)`

### 5.4 业务查询索引
- 外键关联字段索引、常用查询字段索引（状态、日期）、树形结构索引（parent_id）

### 5.5 Flyway迁移脚本
- 命名：`V20260526001__create_prod_product_unit_indexes.sql`
...
| 序号 | 文件路径 | 说明 |
|:---:|---------|------|
| 1 | db/migration/V20260526001__create_prod_product_unit_indexes.sql | prod_product_unit表索引与约束脚本 |
| 2 | db/migration/V20260526001__drop_prod_product_unit_indexes.sql | 索引回滚脚本(DROP INDEX) |
...
| 序号 | 检查项 | 验证方法 |
|:---:|--------|--------|
| 1 | 主键约束pk_prod_product_unit创建成功 | information_schema.table_constraints查询 |
