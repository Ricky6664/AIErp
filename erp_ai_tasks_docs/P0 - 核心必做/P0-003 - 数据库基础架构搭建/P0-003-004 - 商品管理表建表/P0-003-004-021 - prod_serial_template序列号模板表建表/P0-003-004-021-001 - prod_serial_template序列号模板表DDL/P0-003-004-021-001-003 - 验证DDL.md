...
- P0-003-004-021-001-002 编写索引与约束（前序兄弟任务）
...
### 5.1 DDL执行验证
- 执行`SELECT tablename FROM pg_tables WHERE schemaname='public' AND tablename='prod_serial_template'`确认表已创建
- 执行`SELECT column_name, data_type, is_nullable FROM information_schema.columns WHERE table_name='prod_serial_template'`验证字段定义
- 执行`SELECT indexname, indexdef FROM pg_indexes WHERE tablename='prod_serial_template'`验证索引
...
| 序号 | 文件路径 | 说明 |
|:---:|---------|------|
| 1 | db/migration/V20260526001__verify_prod_serial_template.sql | prod_serial_template表DDL验证查询脚本 |
| 2 | docs/verification/prod_serial_template_verify_report.md | 验证报告记录 |
