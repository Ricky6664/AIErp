# P0-003-001-002-001-003 验证规范

## 一、任务标识

| 属性 | 值 |
|------|-----|
| 任务编号 | P0-003-001-002-001-003 |
| 任务名称 | 验证规范 |
| 所属模块 | P0-003 |
| 优先级 | P0 |
| 任务类型 | 验证/测试 |
...
- P0-003-001-002-001-002 编写默认值与约束（前序兄弟任务）
...
### 5.1 DDL执行验证
- 执行`SELECT tablename FROM pg_tables WHERE schemaname='public' AND tablename='chema'`确认表已创建
- 执行`SELECT column_name, data_type, is_nullable FROM information_schema.columns WHERE table_name='chema'`验证字段定义
- 执行`SELECT indexname, indexdef FROM pg_indexes WHERE tablename='chema'`验证索引
...
| 序号 | 文件路径 | 说明 |
|:---:|---------|------|
| 1 | db/migration/V20260526001__verify_chema.sql | chema表DDL验证查询脚本 |
| 2 | docs/verification/chema_verify_report.md | 验证报告记录 |
