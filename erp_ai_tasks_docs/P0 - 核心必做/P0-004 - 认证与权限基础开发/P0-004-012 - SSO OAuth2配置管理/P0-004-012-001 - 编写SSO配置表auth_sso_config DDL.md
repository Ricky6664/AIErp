...
| 属性 | 值 |
|------|-----|
| 任务编号 | P0-004-012-001a |
| 任务名称 | SSO配置表建表（auth_sso_config：sso_name/sso_type(SAML/OAuth2/CAS)/idp_url/sp_entity_id/sso_url/slo_url/certificate/enable_flag） |
| 所属模块 | P0-004 |
| 优先级 | P0 |
| 任务类型 | DDL/数据库建表 |
...
完成SSO配置表建表（auth_sso_config：sso_name/sso_type(SAML/OAuth2/CAS)/idp_url/sp_entity_id/sso_url/slo_url/certificate/enable_flag）的开发工作，确保功能完整、质量达标
