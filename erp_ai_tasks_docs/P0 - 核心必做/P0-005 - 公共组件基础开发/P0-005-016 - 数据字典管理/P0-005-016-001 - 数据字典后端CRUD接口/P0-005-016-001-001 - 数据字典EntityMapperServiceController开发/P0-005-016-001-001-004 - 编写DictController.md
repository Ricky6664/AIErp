# P0-005-016-001-001-004 编写DictController

## 一、任务标识

| 属性 | 值 |
|------|-----|
| 任务编号 | P0-005-016-001-001-004 |
| 任务名称 | 编写DictController |
| 所属模块 | P0-005 |
| 优先级 | P0 |
| 任务类型 | Controller接口层 |
...
@RestController+@RequestMapping"/api/system/dict"+字典类型CRUD+字典数据CRUD+按dict_type_code查询字典数据列表+缓存手动刷新+导入/导出
...
- P0-005-016-001-001-003 编写DictTypeService+DictDataService（主从CRUD+dict_type_code唯一性校验+缓存@Cacheable("sys_dict")+缓存刷新@CacheEvict+批量启用/禁用）（前序兄弟任务）
...
| 序号 | 文件路径 | 说明 |
|:---:|---------|------|
| 1 | erp-system-module/src/main/java/com/erp/system/dict/controller/ | Controller类 |
| 2 | erp-system-module/src/main/java/com/erp/system/dict/controller/ | Swagger接口文档注解 |
