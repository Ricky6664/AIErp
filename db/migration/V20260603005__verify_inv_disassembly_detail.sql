...
-- Version: V20260603005
-- Description: inv_disassembly_detail拆卸主从表DDL验证查询脚本
-- Author: AI
-- Date: 2026-06-03
-- Task: P0-003-007-012-001-003
-- ============================================================

-- ============================================================
-- 一、inv_disassembly 拆卸主表验证
-- ============================================================

-- 1.1 表存在性验证
SELECT 'TABLE_INV_DISASSEMBLY_EXISTS' AS check_name,
       CASE WHEN count(*) > 0 THEN 'PASS' ELSE 'FAIL' END AS result,
       'inv_disassembly table should exist in public schema' AS description
FROM pg_tables
WHERE schemaname = 'public' AND tablename = 'inv_disassembly';
...
WHERE table_name = 'inv_disassembly'
...
actual_fields AS (
    SELECT column_name FROM information_schema.columns WHERE table_name = 'inv_disassembly'
)
...
SELECT 'FIELD_COUNT_INV_DISASSEMBLY' AS check_name,
...
-- 1.5 主键约束验证
SELECT 'PK_INV_DISASSEMBLY_CHECK' AS check_name,
       CASE WHEN count(*) = 1 THEN 'PASS' ELSE 'FAIL' END AS result,
       'inv_disassembly should have exactly 1 primary key' AS description
FROM pg_constraint c
JOIN pg_class t ON c.conrelid = t.oid
WHERE t.relname = 'inv_disassembly' AND c.contype = 'p';
...
FROM pg_indexes
WHERE tablename = 'inv_disassembly'
...
SELECT 'TENANT_INDEX_INV_DISASSEMBLY' AS check_name,
...
-- 1.9 COMMENT注释完整性验证
SELECT 'TABLE_COMMENT_INV_DISASSEMBLY' AS check_name,
       CASE WHEN obj_description('inv_disassembly'::regclass) IS NOT NULL THEN 'PASS' ELSE 'FAIL' END AS result;

SELECT c.column_name,
       col_description('inv_disassembly'::regclass, c.ordinal_position) AS comment,
       CASE WHEN col_description('inv_disassembly'::regclass, c.ordinal_position) IS NULL THEN 'MISSING' ELSE 'OK' END AS status
FROM information_schema.columns c
WHERE c.table_name = 'inv_disassembly'
...
SELECT 'FK_INV_DISASSEMBLY_CHECK' AS check_name,
       CASE WHEN count(*) = 0 THEN 'PASS' ELSE 'FAIL' END AS result,
       'No foreign key constraints should exist (app-layer relationship)' AS description
FROM pg_constraint c
JOIN pg_class t ON c.conrelid = t.oid
WHERE t.relname = 'inv_disassembly' AND c.contype = 'f';

-- 1.12 索引数量验证（期望 17：1 PK + 1 UNIQUE + 15 普通索引）
SELECT 'INDEX_COUNT_INV_DISASSEMBLY' AS check_name,
...
SELECT 'COLUMN_ORDER_NO_CHECK' AS check_name,
       CASE WHEN count(*) > 0 THEN 'PASS' ELSE 'FAIL' END AS result,
       '"order_no" column must exist (unique index uk_inv_disassembly_order_no references it)' AS description
FROM information_schema.columns
WHERE table_name = 'inv_disassembly' AND column_name = 'order_no';
...
-- ============================================================
-- 二、inv_disassembly_detail 拆卸从表验证
-- ============================================================

-- 2.1 表存在性验证
SELECT 'TABLE_INV_DISASSEMBLY_DETAIL_EXISTS' AS check_name,
       CASE WHEN count(*) > 0 THEN 'PASS' ELSE 'FAIL' END AS result,
       'inv_disassembly_detail table should exist in public schema' AS description
FROM pg_tables
WHERE schemaname = 'public' AND tablename = 'inv_disassembly_detail';
...
SELECT column_name,
       ...
       column_default
FROM information_schema.columns
WHERE table_name = 'inv_disassembly_detail'
ORDER BY ordinal_position;
...
actual_fields AS (
    SELECT column_name FROM information_schema.columns WHERE table_name = 'inv_disassembly_detail'
)
SELECT rf.field_name AS missing_field,
       'FAIL' AS result
FROM required_fields rf
...
-- 2.4 字段数量验证（期望 54：10通用 + 10业务 + 12快照/remark + 22扩展）
SELECT 'FIELD_COUNT_INV_DISASSEMBLY_DETAIL' AS check_name,
       CASE WHEN count(*) = 54 THEN 'PASS' ELSE 'FAIL' END AS result,
       'Expected 54 columns (10 common + 10 business + 12 snapshot/remark + 22 extension)' AS description,
       count(*) AS actual_count
FROM information_schema.columns
WHERE table_name = 'inv_disassembly_detail';

-- 2.5 主键约束验证
SELECT 'PK_INV_DISASSEMBLY_DETAIL_CHECK' AS check_name,
       CASE WHEN count(*) = 1 THEN 'PASS' ELSE 'FAIL' END AS result,
       'inv_disassembly_detail should have exactly 1 primary key' AS description
FROM pg_constraint c
JOIN pg_class t ON c.conrelid = t.oid
WHERE t.relname = 'inv_disassembly_detail' AND c.contype = 'p';
...
SELECT indexname,
       ...
       CASE WHEN indexdef ILIKE '%WHERE%is_deleted%false%' THEN 'PASS' ELSE 'FAIL' END AS partial_unique_check
FROM pg_indexes
WHERE tablename = 'inv_disassembly_detail'
  AND indexname LIKE 'uk_%';
...
SELECT 'TENANT_INDEX_INV_DISASSEMBLY_DETAIL' AS check_name,
       CASE WHEN count(*) >= 3 THEN 'PASS' ELSE 'FAIL' END AS result,
       'Expected at least 3 tenant_id-leading indexes' AS description,
       count(*) AS actual_count
FROM pg_indexes
...
-- 2.9 COMMENT注释完整性验证
SELECT 'TABLE_COMMENT_INV_DISASSEMBLY_DETAIL' AS check_name,
       CASE WHEN obj_description('inv_disassembly_detail'::regclass) IS NOT NULL THEN 'PASS' ELSE 'FAIL' END AS result;

SELECT c.column_name,
       col_description('inv_disassembly_detail'::regclass, c.ordinal_position) AS comment,
       CASE WHEN col_description('inv_disassembly_detail'::regclass, c.ordinal_position) IS NULL THEN 'MISSING' ELSE 'OK' END AS status
FROM information_schema.columns c
WHERE c.table_name = 'inv_disassembly_detail'
ORDER BY c.ordinal_position;
...
SELECT 'FK_INV_DISASSEMBLY_DETAIL_CHECK' AS check_name,
       CASE WHEN count(*) = 0 THEN 'PASS' ELSE 'FAIL' END AS result,
       'No foreign key constraints should exist (app-layer relationship)' AS description
FROM pg_constraint c
JOIN pg_class t ON c.conrelid = t.oid
WHERE t.relname = 'inv_disassembly_detail' AND c.contype = 'f';

-- 2.12 索引数量验证（期望 17：1 PK + 1 UNIQUE + 15 普通索引）
SELECT 'INDEX_COUNT_INV_DISASSEMBLY_DETAIL' AS check_name,
       CASE WHEN count(*) = 17 THEN 'PASS' ELSE 'FAIL' END AS result,
       'Expected 17 indexes (1 PK + 1 unique + 15 standard)' AS description,
       count(*) AS actual_count
FROM pg_indexes
...
WHERE script LIKE '%inv_disassembly%'
