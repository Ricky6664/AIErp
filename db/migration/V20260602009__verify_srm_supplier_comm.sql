...
-- Version: V20260602009
-- Description: srm_supplier_comm 供应商联系人通讯表DDL验证查询
-- Author: AI Generated
-- Date: 2026-06-02
-- Task: P0-003-006-004-001-003
-- ============================================================
...
WHERE schemaname = 'public'
  AND tablename = 'srm_supplier_comm';
...
WHERE table_name = 'srm_supplier_comm'
...
FROM pg_indexes
WHERE tablename = 'srm_supplier_comm'
ORDER BY indexname;
...
WHERE rel.relname = 'srm_supplier_comm'
...
FROM pg_class c
WHERE c.relname = 'srm_supplier_comm'
...
FROM pg_attribute a
WHERE a.attrelid = 'srm_supplier_comm'::regclass
  AND a.attnum > 0
...
WHERE script LIKE '%srm_supplier_comm%'
...
BEGIN
    ...
    FROM pg_indexes
    WHERE tablename = 'srm_supplier_comm'
      AND indexdef LIKE '%ON srm_supplier_comm USING btree (tenant_id%';

    RAISE NOTICE '===== srm_supplier_comm DDL验证结果汇总 =====';
