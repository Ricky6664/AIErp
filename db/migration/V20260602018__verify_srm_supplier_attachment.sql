...
-- Version: V20260602018
-- Description: srm_supplier_attachment 供应商附件表DDL验证查询
-- Author: AI Generated
-- Date: 2026-06-02
-- Task: P0-003-006-007-001-003
-- ============================================================
...
WHERE schemaname = 'public'
  AND tablename = 'srm_supplier_attachment';
...
WHERE table_name = 'srm_supplier_attachment'
...
FROM pg_indexes
WHERE tablename = 'srm_supplier_attachment'
ORDER BY indexname;
...
WHERE rel.relname = 'srm_supplier_attachment'
...
FROM pg_class c
WHERE c.relname = 'srm_supplier_attachment'
...
FROM pg_attribute a
WHERE a.attrelid = 'srm_supplier_attachment'::regclass
  AND a.attnum > 0
...
WHERE script LIKE '%srm_supplier_attachment%'
...
BEGIN
    ...
    FROM pg_indexes
    WHERE tablename = 'srm_supplier_attachment'
      AND indexdef LIKE '%ON srm_supplier_attachment USING btree (tenant_id%';

    RAISE NOTICE '===== srm_supplier_attachment DDL验证结果汇总 =====';
    RAISE NOTICE '表存在: % (期望: 1)', v_table_count;
    RAISE NOTICE '字段总数: % (期望: 41)', v_col_count;
    RAISE NOTICE '索引总数: % (期望: 12)', v_idx_count;
    RAISE NOTICE '已注释字段: % (期望: 19)', v_cmt_count;
