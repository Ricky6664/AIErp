...
-- Version: V20260601047
-- Description: prod_standard_process 标准工序表DDL验证查询脚本
-- Author: AI Generated
-- Date: 2026-06-01
-- Task: P0-003-004-013-001-003
-- ============================================================
...
SELECT 'TABLE_EXISTS_CHECK' AS check_name,
       ...
       'prod_standard_process table should exist in public schema' AS description
FROM pg_tables
WHERE schemaname = 'public' AND tablename = 'prod_standard_process';
...
WHERE table_name = 'prod_standard_process'
...
actual_fields AS (
    SELECT column_name FROM information_schema.columns WHERE table_name = 'prod_standard_process'
)
...
SELECT 'PK_CHECK' AS check_name,
       ...
       'prod_standard_process should have exactly 1 primary key' AS description
FROM pg_constraint c
JOIN pg_class t ON c.conrelid = t.oid
WHERE t.relname = 'prod_standard_process' AND c.contype = 'p';
...
FROM pg_indexes
WHERE tablename = 'prod_standard_process'
...
SELECT 'TABLE_COMMENT_CHECK' AS check_name,
       CASE WHEN obj_description('prod_standard_process'::regclass) IS NOT NULL THEN 'PASS' ELSE 'FAIL' END AS result;

SELECT c.column_name,
       col_description('prod_standard_process'::regclass, c.ordinal_position) AS comment,
       CASE WHEN col_description('prod_standard_process'::regclass, c.ordinal_position) IS NULL THEN 'MISSING' ELSE 'OK' END AS status
FROM information_schema.columns c
WHERE c.table_name = 'prod_standard_process'
...
SELECT column_name,
       is_nullable,
       CASE WHEN column_name IN ('id', 'tenant_id', 'code', 'name', 'currency_code', 'status', 'created_at', 'updated_at', 'is_deleted', 'version')
             AND is_nullable = 'NO' THEN 'PASS'
            WHEN column_name IN ('id', 'tenant_id', 'code', 'name', 'currency_code', 'status', 'created_at', 'updated_at', 'is_deleted', 'version')
             AND is_nullable = 'YES' THEN 'FAIL'
            ELSE 'OK' END AS result
FROM information_schema.columns
WHERE table_name = 'prod_standard_process'
  AND column_name IN ('id', 'tenant_id', 'code', 'name', 'currency_code', 'status', 'created_at', 'updated_at', 'is_deleted', 'version');
...
WHERE script LIKE '%prod_standard_process%'
...
FROM pg_constraint c
JOIN pg_class t ON c.conrelid = t.oid
WHERE t.relname = 'prod_standard_process' AND c.contype = 'f';
...
SELECT 'COLUMN_CODE_CHECK' AS check_name,
       CASE WHEN count(*) > 0 THEN 'PASS' ELSE 'FAIL' END AS result,
       '"code" column must exist (unique index uk_prod_standard_process_code references it)' AS description
FROM information_schema.columns
WHERE table_name = 'prod_standard_process' AND column_name = 'code';

SELECT 'COLUMN_STATUS_CHECK' AS check_name,
       CASE WHEN count(*) > 0 THEN 'PASS' ELSE 'FAIL' END AS result,
       '"status" column must exist (indexes reference it)' AS description
FROM information_schema.columns
WHERE table_name = 'prod_standard_process' AND column_name = 'status';

SELECT 'COLUMN_PROCESS_TYPE_CHECK' AS check_name,
       CASE WHEN count(*) > 0 THEN 'PASS' ELSE 'FAIL' END AS result,
       '"process_type" column must exist (index references it)' AS description
FROM information_schema.columns
WHERE table_name = 'prod_standard_process' AND column_name = 'process_type';
...
FROM information_schema.columns
WHERE table_name = 'prod_standard_process'
  AND column_name IN ('code', 'name', 'process_type', 'currency_code', 'description', 'remark');
...
-- ============================================================
-- 19. 业务字段存在性验证（10个业务字段）
-- ============================================================
WITH required_biz_fields AS (
    SELECT unnest(ARRAY[
        'code', 'name', 'process_type', 'standard_hours',
        'standard_cost', 'unit_price', 'currency_code',
...
-- ============================================================
SELECT indexname,
       CASE WHEN indexname ~ '^(pk_|uk_|idx_)prod_standard_process' THEN 'PASS'
            WHEN indexname = 'prod_standard_process_pkey' THEN 'OK'
