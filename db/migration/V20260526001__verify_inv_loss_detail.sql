...
-- Description: inv_loss_detail报损主从表DDL验证查询脚本
-- Author: AI Generated
-- Date: 2026-06-02
-- Task: P0-003-007-009-001-003
-- ============================================================
...
-- 1.1 表存在性验证
SELECT 'TABLE_INV_LOSS_EXISTS' AS check_name,
       CASE WHEN count(*) > 0 THEN 'PASS' ELSE 'FAIL' END AS result,
       'inv_loss table should exist in public schema' AS description
FROM pg_tables
WHERE schemaname = 'public' AND tablename = 'inv_loss';
...
actual_fields AS (
    SELECT column_name FROM information_schema.columns WHERE table_name = 'inv_loss'
)
...
SELECT 'FIELD_COUNT_INV_LOSS' AS check_name,
...
-- 1.5 主键约束验证
SELECT 'PK_INV_LOSS_CHECK' AS check_name,
       CASE WHEN count(*) = 1 THEN 'PASS' ELSE 'FAIL' END AS result,
       'inv_loss should have exactly 1 primary key' AS description
FROM pg_constraint c
JOIN pg_class t ON c.conrelid = t.oid
WHERE t.relname = 'inv_loss' AND c.contype = 'p';
...
SELECT 'TENANT_INDEX_INV_LOSS' AS check_name,
...
-- 1.9 COMMENT注释完整性验证
SELECT 'TABLE_COMMENT_INV_LOSS' AS check_name,
       CASE WHEN obj_description('inv_loss'::regclass) IS NOT NULL THEN 'PASS' ELSE 'FAIL' END AS result;

SELECT c.column_name,
       col_description('inv_loss'::regclass, c.ordinal_position) AS comment,
       CASE WHEN col_description('inv_loss'::regclass, c.ordinal_position) IS NULL THEN 'MISSING' ELSE 'OK' END AS status
FROM information_schema.columns c
WHERE c.table_name = 'inv_loss'
...
SELECT 'FK_INV_LOSS_CHECK' AS check_name,
       CASE WHEN count(*) = 0 THEN 'PASS' ELSE 'FAIL' END AS result,
       'No foreign key constraints should exist (app-layer relationship)' AS description
FROM pg_constraint c
JOIN pg_class t ON c.conrelid = t.oid
WHERE t.relname = 'inv_loss' AND c.contype = 'f';

-- 1.12 索引数量验证（期望 17：1 PK + 1 UNIQUE + 15 普通索引）
SELECT 'INDEX_COUNT_INV_LOSS' AS check_name,
...
SELECT 'COLUMN_ORDER_NO_CHECK' AS check_name,
       CASE WHEN count(*) > 0 THEN 'PASS' ELSE 'FAIL' END AS result,
       '"order_no" column must exist (unique index uk_inv_loss_order_no references it)' AS description
FROM information_schema.columns
WHERE table_name = 'inv_loss' AND column_name = 'order_no';
...
-- ============================================================
-- 二、inv_loss_detail 报损从表验证
-- ============================================================

-- 2.1 表存在性验证
SELECT 'TABLE_INV_LOSS_DETAIL_EXISTS' AS check_name,
       CASE WHEN count(*) > 0 THEN 'PASS' ELSE 'FAIL' END AS result,
       'inv_loss_detail table should exist in public schema' AS description
FROM pg_tables
WHERE schemaname = 'public' AND tablename = 'inv_loss_detail';
...
SELECT column_name,
       ...
       column_default
FROM information_schema.columns
WHERE table_name = 'inv_loss_detail'
ORDER BY ordinal_position;
...
actual_fields AS (
    SELECT column_name FROM information_schema.columns WHERE table_name = 'inv_loss_detail'
)
SELECT rf.field_name AS missing_field,
       'FAIL' AS result
FROM required_fields rf
...
-- 2.4 字段数量验证（期望 54：11业务 + 11商品快照 + 1remark + 22扩展 + 10通用 - 1重叠）
-- 实际：10通用 + 11业务 + 11快照 + 22扩展 = 54
SELECT 'FIELD_COUNT_INV_LOSS_DETAIL' AS check_name,
...
-- 2.5 主键约束验证
SELECT 'PK_INV_LOSS_DETAIL_CHECK' AS check_name,
       CASE WHEN count(*) = 1 THEN 'PASS' ELSE 'FAIL' END AS result,
       'inv_loss_detail should have exactly 1 primary key' AS description
FROM pg_constraint c
JOIN pg_class t ON c.conrelid = t.oid
WHERE t.relname = 'inv_loss_detail' AND c.contype = 'p';
...
SELECT indexname,
       ...
       CASE WHEN indexdef ILIKE '%WHERE%is_deleted%false%' THEN 'PASS' ELSE 'FAIL' END AS partial_unique_check
FROM pg_indexes
WHERE tablename = 'inv_loss_detail'
  AND indexname LIKE 'uk_%';
...
SELECT 'TENANT_INDEX_INV_LOSS_DETAIL' AS check_name,
...
-- 2.9 COMMENT注释完整性验证
SELECT 'TABLE_COMMENT_INV_LOSS_DETAIL' AS check_name,
       CASE WHEN obj_description('inv_loss_detail'::regclass) IS NOT NULL THEN 'PASS' ELSE 'FAIL' END AS result;

SELECT c.column_name,
       col_description('inv_loss_detail'::regclass, c.ordinal_position) AS comment,
       CASE WHEN col_description('inv_loss_detail'::regclass, c.ordinal_position) IS NULL THEN 'MISSING' ELSE 'OK' END AS status
FROM information_schema.columns c
WHERE c.table_name = 'inv_loss_detail'
ORDER BY c.ordinal_position;
...
SELECT 'FK_INV_LOSS_DETAIL_CHECK' AS check_name,
       CASE WHEN count(*) = 0 THEN 'PASS' ELSE 'FAIL' END AS result,
       'No foreign key constraints should exist (app-layer relationship)' AS description
FROM pg_constraint c
JOIN pg_class t ON c.conrelid = t.oid
WHERE t.relname = 'inv_loss_detail' AND c.contype = 'f';

-- 2.12 索引数量验证（期望 17：1 PK + 1 UNIQUE + 15 普通索引）
SELECT 'INDEX_COUNT_INV_LOSS_DETAIL' AS check_name,
       CASE WHEN count(*) = 17 THEN 'PASS' ELSE 'FAIL' END AS result,
       'Expected 17 indexes (1 PK + 1 unique + 15 standard)' AS description,
       count(*) AS actual_count
FROM pg_indexes
