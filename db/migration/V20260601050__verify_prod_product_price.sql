...
-- Version: V20260601050
-- Description: prod_product_price 商品价格表DDL验证查询脚本
-- Author: AI Generated
-- Date: 2026-06-01
-- Task: P0-003-004-014-001-003
-- ============================================================
...
SELECT 'TABLE_EXISTS_CHECK' AS check_name,
       ...
       'prod_product_price table should exist in public schema' AS description
FROM pg_tables
WHERE schemaname = 'public' AND tablename = 'prod_product_price';
...
WHERE table_name = 'prod_product_price'
...
actual_fields AS (
    SELECT column_name FROM information_schema.columns WHERE table_name = 'prod_product_price'
)
...
SELECT 'PK_CHECK' AS check_name,
       ...
       'prod_product_price should have exactly 1 primary key' AS description
FROM pg_constraint c
JOIN pg_class t ON c.conrelid = t.oid
WHERE t.relname = 'prod_product_price' AND c.contype = 'p';
...
FROM pg_indexes
WHERE tablename = 'prod_product_price'
...
SELECT 'TABLE_COMMENT_CHECK' AS check_name,
       CASE WHEN obj_description('prod_product_price'::regclass) IS NOT NULL THEN 'PASS' ELSE 'FAIL' END AS result;

SELECT c.column_name,
       col_description('prod_product_price'::regclass, c.ordinal_position) AS comment,
       CASE WHEN col_description('prod_product_price'::regclass, c.ordinal_position) IS NULL THEN 'MISSING' ELSE 'OK' END AS status
FROM information_schema.columns c
WHERE c.table_name = 'prod_product_price'
...
SELECT column_name,
       is_nullable,
       CASE WHEN column_name IN ('id', 'tenant_id', 'code', 'name', 'product_id', 'price', 'currency_code', 'is_default', 'status', 'created_at', 'updated_at', 'is_deleted', 'version')
             AND is_nullable = 'NO' THEN 'PASS'
            WHEN column_name IN ('id', 'tenant_id', 'code', 'name', 'product_id', 'price', 'currency_code', 'is_default', 'status', 'created_at', 'updated_at', 'is_deleted', 'version')
             AND is_nullable = 'YES' THEN 'FAIL'
            ELSE 'OK' END AS result
FROM information_schema.columns
WHERE table_name = 'prod_product_price'
  AND column_name IN ('id', 'tenant_id', 'code', 'name', 'product_id', 'price', 'currency_code', 'is_default', 'status', 'created_at', 'updated_at', 'is_deleted', 'version');
...
WHERE script LIKE '%prod_product_price%'
...
FROM pg_constraint c
JOIN pg_class t ON c.conrelid = t.oid
WHERE t.relname = 'prod_product_price' AND c.contype = 'f';

-- ============================================================
-- 13. 索引数量验证（期望 12：1 PK + 1 UK + 10 普通索引）
-- ============================================================
...
SELECT 'COLUMN_CODE_CHECK' AS check_name,
       CASE WHEN count(*) > 0 THEN 'PASS' ELSE 'FAIL' END AS result,
       '"code" column must exist (unique index uk_prod_product_price_code references it)' AS description
FROM information_schema.columns
WHERE table_name = 'prod_product_price' AND column_name = 'code';

SELECT 'COLUMN_STATUS_CHECK' AS check_name,
       CASE WHEN count(*) > 0 THEN 'PASS' ELSE 'FAIL' END AS result,
       '"status" column must exist (indexes reference it)' AS description
FROM information_schema.columns
WHERE table_name = 'prod_product_price' AND column_name = 'status';

SELECT 'COLUMN_PRODUCT_ID_CHECK' AS check_name,
       CASE WHEN count(*) > 0 THEN 'PASS' ELSE 'FAIL' END AS result,
       '"product_id" column must exist (index references it)' AS description
FROM information_schema.columns
WHERE table_name = 'prod_product_price' AND column_name = 'product_id';

SELECT 'COLUMN_PRICE_TYPE_CHECK' AS check_name,
       CASE WHEN count(*) > 0 THEN 'PASS' ELSE 'FAIL' END AS result,
       '"price_type" column must exist (index references it)' AS description
FROM information_schema.columns
WHERE table_name = 'prod_product_price' AND column_name = 'price_type';

SELECT 'COLUMN_UNIT_ID_CHECK' AS check_name,
       CASE WHEN count(*) > 0 THEN 'PASS' ELSE 'FAIL' END AS result,
       '"unit_id" column must exist (index references it)' AS description
FROM information_schema.columns
WHERE table_name = 'prod_product_price' AND column_name = 'unit_id';
...
-- ============================================================
-- 16. DECIMAL(18,8)字段精度验证（price金额字段）
-- ============================================================
SELECT column_name,
       data_type,
       numeric_precision,
...
SELECT column_name,
       ...
       CASE WHEN column_name = 'code' AND character_maximum_length = 50 THEN 'PASS'
            ...
            WHEN column_name = 'price_type' AND character_maximum_length = 50 THEN 'PASS'
...
FROM information_schema.columns
WHERE table_name = 'prod_product_price'
  AND column_name IN ('code', 'name', 'price_type', 'currency_code', 'remark');
...
FROM information_schema.columns
WHERE table_name = 'prod_product_price'
  AND column_name IN ('currency_code', 'status', 'is_deleted', 'is_default', 'version', 'created_at', 'updated_at');

-- ============================================================
-- 19. 业务字段存在性验证（12个业务字段）
-- ============================================================
WITH required_biz_fields AS (
    SELECT unnest(ARRAY[
        'code', 'name', 'product_id', 'price',
        'price_type', 'currency_code', 'unit_id',
        'is_default', 'effective_date', 'expiry_date',
        'status', 'remark'
    ]) AS field_name
),
actual_fields AS (
...
-- ============================================================
SELECT indexname,
       CASE WHEN indexname ~ '^(pk_|uk_|idx_)prod_product_price' THEN 'PASS'
            WHEN indexname = 'prod_product_price_pkey' THEN 'OK'
