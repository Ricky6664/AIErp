...
-- Version: V20260602021
-- Description: srm_supplier_evaluation 供应商评价表DDL验证查询
-- Author: AI Generated
-- Date: 2026-06-02
-- Task: P0-003-006-008-001-003
-- ============================================================
...
WHERE schemaname = 'public'
  AND tablename = 'srm_supplier_evaluation';
...
WHERE table_name = 'srm_supplier_evaluation'
...
FROM pg_indexes
WHERE tablename = 'srm_supplier_evaluation'
ORDER BY indexname;
...
WHERE rel.relname = 'srm_supplier_evaluation'
...
FROM pg_class c
WHERE c.relname = 'srm_supplier_evaluation'
...
FROM pg_attribute a
WHERE a.attrelid = 'srm_supplier_evaluation'::regclass
  AND a.attnum > 0
...
WHERE script LIKE '%srm_supplier_evaluation%'
...
BEGIN
    ...
    FROM pg_indexes
    WHERE tablename = 'srm_supplier_evaluation'
      AND indexdef LIKE '%ON srm_supplier_evaluation USING btree (tenant_id%';

    RAISE NOTICE '===== srm_supplier_evaluation DDL验证结果汇总 =====';
    RAISE NOTICE '表存在: % (期望: 1)', v_table_count;
    RAISE NOTICE '字段总数: % (期望: 43)', v_col_count;
    RAISE NOTICE '索引总数: % (期望: 15)', v_idx_count;
    RAISE NOTICE '已注释字段: % (期望: 20)', v_cmt_count;
