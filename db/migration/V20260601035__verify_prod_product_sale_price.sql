...
-- Version: V20260601035
-- Description: prod_product_sale_price商品销价核定表DDL验证查询脚本
-- Author: AI Generated
-- Date: 2026-06-01
-- ============================================================
...
SELECT 'TABLE_EXISTS_CHECK' AS check_name,
       ...
       'prod_product_sale_price table should exist in public schema' AS description
FROM pg_tables
WHERE schemaname = 'public' AND tablename = 'prod_product_sale_price';
...
WHERE table_name = 'prod_product_sale_price'
...
actual_fields AS (
    SELECT column_name FROM information_schema.columns WHERE table_name = 'prod_product_sale_price'
)
...
SELECT 'PK_CHECK' AS check_name,
       ...
       'prod_product_sale_price should have exactly 1 primary key' AS description
FROM pg_constraint c
JOIN pg_class t ON c.conrelid = t.oid
WHERE t.relname = 'prod_product_sale_price' AND c.contype = 'p';
...
FROM pg_indexes
WHERE tablename = 'prod_product_sale_price'
...
SELECT 'TABLE_COMMENT_CHECK' AS check_name,
       CASE WHEN obj_description('prod_product_sale_price'::regclass) IS NOT NULL THEN 'PASS' ELSE 'FAIL' END AS result;

SELECT c.column_name,
       col_description('prod_product_sale_price'::regclass, c.ordinal_position) AS comment,
       CASE WHEN col_description('prod_product_sale_price'::regclass, c.ordinal_position) IS NULL THEN 'MISSING' ELSE 'OK' END AS status
FROM information_schema.columns c
WHERE c.table_name = 'prod_product_sale_price'
...
SELECT column_name,
       is_nullable,
       CASE WHEN column_name IN ('id', 'tenant_id', 'product_id', 'sale_price', 'currency_code', 'is_default', 'status', 'created_at', 'updated_at', 'is_deleted', 'version')
             AND is_nullable = 'NO' THEN 'PASS'
            WHEN column_name IN ('id', 'tenant_id', 'product_id', 'sale_price', 'currency_code', 'is_default', 'status', 'created_at', 'updated_at', 'is_deleted', 'version')
             AND is_nullable = 'YES' THEN 'FAIL'
            ELSE 'OK' END AS result
FROM information_schema.columns
WHERE table_name = 'prod_product_sale_price'
  AND column_name IN ('id', 'tenant_id', 'product_id', 'sale_price', 'currency_code', 'is_default', 'status', 'created_at', 'updated_at', 'is_deleted', 'version');
...
WHERE script LIKE '%prod_product_sale_price%'
...
FROM pg_constraint c
JOIN pg_class t ON c.conrelid = t.oid
WHERE t.relname = 'prod_product_sale_price' AND c.contype = 'f';
...
SELECT 'COLUMN_PRODUCT_ID_CHECK' AS check_name,
       CASE WHEN count(*) > 0 THEN 'PASS' ELSE 'FAIL' END AS result,
       '"product_id" column must exist (unique index uk_prod_product_sale_price_tenant_product_customer references it)' AS description
FROM information_schema.columns
WHERE table_name = 'prod_product_sale_price' AND column_name = 'product_id';

SELECT 'COLUMN_CUSTOMER_ID_CHECK' AS check_name,
       CASE WHEN count(*) > 0 THEN 'PASS' ELSE 'FAIL' END AS result,
       '"customer_id" column must exist (unique index uk_prod_product_sale_price_tenant_product_customer references it)' AS description
FROM information_schema.columns
WHERE table_name = 'prod_product_sale_price' AND column_name = 'customer_id';

SELECT 'COLUMN_STATUS_CHECK' AS check_name,
       CASE WHEN count(*) > 0 THEN 'PASS' ELSE 'FAIL' END AS result,
       '"status" column must exist (indexes reference it)' AS description
FROM information_schema.columns
WHERE table_name = 'prod_product_sale_price' AND column_name = 'status';

SELECT 'COLUMN_CURRENCY_CODE_CHECK' AS check_name,
       CASE WHEN count(*) > 0 THEN 'PASS' ELSE 'FAIL' END AS result,
       '"currency_code" column must exist (index references it)' AS description
FROM information_schema.columns
WHERE table_name = 'prod_product_sale_price' AND column_name = 'currency_code';

SELECT 'COLUMN_UNIT_ID_CHECK' AS check_name,
       CASE WHEN count(*) > 0 THEN 'PASS' ELSE 'FAIL' END AS result,
       '"unit_id" column must exist (index references it)' AS description
FROM information_schema.columns
WHERE table_name = 'prod_product_sale_price' AND column_name = 'unit_id';

SELECT 'COLUMN_EFFECTIVE_DATE_CHECK' AS check_name,
       CASE WHEN count(*) > 0 THEN 'PASS' ELSE 'FAIL' END AS result,
       '"effective_date" column must exist (index references it)' AS description
FROM information_schema.columns
WHERE table_name = 'prod_product_sale_price' AND column_name = 'effective_date';

SELECT 'COLUMN_IS_DEFAULT_CHECK' AS check_name,
       CASE WHEN count(*) > 0 THEN 'PASS' ELSE 'FAIL' END AS result,
       '"is_default" column must exist (index references it)' AS description
FROM information_schema.columns
WHERE table_name = 'prod_product_sale_price' AND column_name = 'is_default';
...
FROM information_schema.columns
WHERE table_name = 'prod_product_sale_price'
  AND column_name IN ('sale_price', 'tax_inclusive_price', 'tax_exclusive_price', 'tax_rate');
...
WITH required_biz_fields AS (
    SELECT unnest(ARRAY[
        'product_id', 'customer_id', 'sale_price', 'tax_inclusive_price',
...
-- ============================================================
SELECT indexname,
       CASE WHEN indexname ~ '^(pk_|uk_|idx_)prod_product_sale_price' THEN 'PASS'
