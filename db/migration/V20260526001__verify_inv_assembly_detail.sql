...
-- Description: inv_assembly_detail组装主从表DDL验证查询脚本
-- Author: AI
-- Date: 2026-06-03
-- Task: P0-003-007-011-001-003
-- ============================================================

-- ============================================================
-- 一、inv_assembly 组装主表验证
-- ============================================================

-- 1.1 表存在性验证
SELECT 'TABLE_INV_ASSEMBLY_EXISTS' AS check_name,
       CASE WHEN count(*) > 0 THEN 'PASS' ELSE 'FAIL' END AS result,
       'inv_assembly table should exist in public schema' AS description
FROM pg_tables
WHERE schemaname = 'public' AND tablename = 'inv_assembly';
...
actual_fields AS (
    SELECT column_name FROM information_schema.columns WHERE table_name = 'inv_assembly'
)
...
SELECT 'FIELD_COUNT_INV_ASSEMBLY' AS check_name,
...
-- 1.5 主键约束验证
SELECT 'PK_INV_ASSEMBLY_CHECK' AS check_name,
       CASE WHEN count(*) = 1 THEN 'PASS' ELSE 'FAIL' END AS result,
       'inv_assembly should have exactly 1 primary key' AS description
FROM pg_constraint c
JOIN pg_class t ON c.conrelid = t.oid
WHERE t.relname = 'inv_assembly' AND c.contype = 'p';
...
SELECT 'TENANT_INDEX_INV_ASSEMBLY' AS check_name,
...
-- 1.9 COMMENT注释完整性验证
SELECT 'TABLE_COMMENT_INV_ASSEMBLY' AS check_name,
       CASE WHEN obj_description('inv_assembly'::regclass) IS NOT NULL THEN 'PASS' ELSE 'FAIL' END AS result;

SELECT c.column_name,
       col_description('inv_assembly'::regclass, c.ordinal_position) AS comment,
       CASE WHEN col_description('inv_assembly'::regclass, c.ordinal_position) IS NULL THEN 'MISSING' ELSE 'OK' END AS status
FROM information_schema.columns c
WHERE c.table_name = 'inv_assembly'
...
SELECT 'FK_INV_ASSEMBLY_CHECK' AS check_name,
       CASE WHEN count(*) = 0 THEN 'PASS' ELSE 'FAIL' END AS result,
       'No foreign key constraints should exist (app-layer relationship)' AS description
FROM pg_constraint c
JOIN pg_class t ON c.conrelid = t.oid
WHERE t.relname = 'inv_assembly' AND c.contype = 'f';

-- 1.12 索引数量验证（期望 17：1 PK + 1 UNIQUE + 15 普通索引）
SELECT 'INDEX_COUNT_INV_ASSEMBLY' AS check_name,
...
SELECT 'COLUMN_ORDER_NO_CHECK' AS check_name,
       CASE WHEN count(*) > 0 THEN 'PASS' ELSE 'FAIL' END AS result,
       '"order_no" column must exist (unique index uk_inv_assembly_order_no references it)' AS description
FROM information_schema.columns
WHERE table_name = 'inv_assembly' AND column_name = 'order_no';
...
-- ============================================================
-- 二、inv_assembly_detail 组装从表验证
-- ============================================================

-- 2.1 表存在性验证
SELECT 'TABLE_INV_ASSEMBLY_DETAIL_EXISTS' AS check_name,
       CASE WHEN count(*) > 0 THEN 'PASS' ELSE 'FAIL' END AS result,
       'inv_assembly_detail table should exist in public schema' AS description
FROM pg_tables
WHERE schemaname = 'public' AND tablename = 'inv_assembly_detail';
...
SELECT column_name,
       ...
       column_default
FROM information_schema.columns
WHERE table_name = 'inv_assembly_detail'
ORDER BY ordinal_position;
...
actual_fields AS (
    SELECT column_name FROM information_schema.columns WHERE table_name = 'inv_assembly_detail'
)
SELECT rf.field_name AS missing_field,
       'FAIL' AS result
FROM required_fields rf
...
SELECT 'FIELD_COUNT_INV_ASSEMBLY_DETAIL' AS check_name,
...
-- 2.5 主键约束验证
SELECT 'PK_INV_ASSEMBLY_DETAIL_CHECK' AS check_name,
       CASE WHEN count(*) = 1 THEN 'PASS' ELSE 'FAIL' END AS result,
       'inv_assembly_detail should have exactly 1 primary key' AS description
FROM pg_constraint c
JOIN pg_class t ON c.conrelid = t.oid
WHERE t.relname = 'inv_assembly_detail' AND c.contype = 'p';
...
SELECT indexname,
       ...
       CASE WHEN indexdef ILIKE '%WHERE%is_deleted%false%' THEN 'PASS' ELSE 'FAIL' END AS partial_unique_check
FROM pg_indexes
WHERE tablename = 'inv_assembly_detail'
  AND indexname LIKE 'uk_%';
...
SELECT 'TENANT_INDEX_INV_ASSEMBLY_DETAIL' AS check_name,
...
-- 2.9 COMMENT注释完整性验证
SELECT 'TABLE_COMMENT_INV_ASSEMBLY_DETAIL' AS check_name,
       CASE WHEN obj_description('inv_assembly_detail'::regclass) IS NOT NULL THEN 'PASS' ELSE 'FAIL' END AS result;

SELECT c.column_name,
       col_description('inv_assembly_detail'::regclass, c.ordinal_position) AS comment,
       CASE WHEN col_description('inv_assembly_detail'::regclass, c.ordinal_position) IS NULL THEN 'MISSING' ELSE 'OK' END AS status
FROM information_schema.columns c
WHERE c.table_name = 'inv_assembly_detail'
ORDER BY c.ordinal_position;
...
SELECT 'FK_INV_ASSEMBLY_DETAIL_CHECK' AS check_name,
       CASE WHEN count(*) = 0 THEN 'PASS' ELSE 'FAIL' END AS result,
       'No foreign key constraints should exist (app-layer relationship)' AS description
FROM pg_constraint c
JOIN pg_class t ON c.conrelid = t.oid
WHERE t.relname = 'inv_assembly_detail' AND c.contype = 'f';

-- 2.12 索引数量验证（期望 17：1 PK + 1 UNIQUE + 15 普通索引）
SELECT 'INDEX_COUNT_INV_ASSEMBLY_DETAIL' AS check_name,
       CASE WHEN count(*) = 17 THEN 'PASS' ELSE 'FAIL' END AS result,
       'Expected 17 indexes (1 PK + 1 unique + 15 standard)' AS description,
       count(*) AS actual_count
FROM pg_indexes
...
WHERE script LIKE '%inv_assembly%'
