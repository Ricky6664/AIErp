...
-- Description: org_department部门表DDL验证查询脚本
-- Author: AI Generated
-- Date: 2026-06-01
-- ============================================================
...
SELECT 'TABLE_EXISTS_CHECK' AS check_name,
       ...
       'org_department table should exist in public schema' AS description
FROM pg_tables
WHERE schemaname = 'public' AND tablename = 'org_department';
...
WHERE table_name = 'org_department'
...
actual_fields AS (
    SELECT column_name FROM information_schema.columns WHERE table_name = 'org_department'
)
...
-- ============================================================
-- 4. 字段数量验证（期望 >= 38：6业务 + 10 ext_str + 5 ext_num + 3 ext_date + 3 ext_bool + 1 ext_json + 10 通用）
-- ============================================================
...
SELECT 'PK_CHECK' AS check_name,
       ...
       'org_department should have exactly 1 primary key' AS description
FROM pg_constraint c
JOIN pg_class t ON c.conrelid = t.oid
WHERE t.relname = 'org_department' AND c.contype = 'p';
...
FROM pg_indexes
WHERE tablename = 'org_department'
...
SELECT 'TABLE_COMMENT_CHECK' AS check_name,
       CASE WHEN obj_description('org_department'::regclass) IS NOT NULL THEN 'PASS' ELSE 'FAIL' END AS result;

SELECT c.column_name,
       col_description('org_department'::regclass, c.ordinal_position) AS comment,
       CASE WHEN col_description('org_department'::regclass, c.ordinal_position) IS NULL THEN 'MISSING' ELSE 'OK' END AS status
FROM information_schema.columns c
WHERE c.table_name = 'org_department'
...
SELECT column_name,
       is_nullable,
       CASE WHEN column_name IN ('id', 'tenant_id', 'dept_code', 'dept_name', 'status', 'created_at', 'updated_at', 'is_deleted', 'version')
             AND is_nullable = 'NO' THEN 'PASS'
            WHEN column_name IN ('id', 'tenant_id', 'dept_code', 'dept_name', 'status', 'created_at', 'updated_at', 'is_deleted', 'version')
             AND is_nullable = 'YES' THEN 'FAIL'
            ELSE 'OK' END AS result
FROM information_schema.columns
WHERE table_name = 'org_department'
  AND column_name IN ('id', 'tenant_id', 'dept_code', 'dept_name', 'status', 'created_at', 'updated_at', 'is_deleted', 'version');
...
WHERE script LIKE '%org_department%'
...
FROM pg_constraint c
JOIN pg_class t ON c.conrelid = t.oid
WHERE t.relname = 'org_department' AND c.contype = 'f';
