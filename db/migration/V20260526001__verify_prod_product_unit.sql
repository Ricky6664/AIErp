...
-- Description: prod_product_unit商品多单位表DDL验证查询脚本
-- Author: AI Generated
-- Date: 2026-06-01
-- ============================================================
...
SELECT 'TABLE_EXISTS_CHECK' AS check_name,
       ...
       'prod_product_unit table should exist in public schema' AS description
FROM pg_tables
WHERE schemaname = 'public' AND tablename = 'prod_product_unit';
...
WHERE table_name = 'prod_product_unit'
...
actual_fields AS (
    SELECT column_name FROM information_schema.columns WHERE table_name = 'prod_product_unit'
)
...
-- ============================================================
-- 5. 数值精度验证（conversion_rate应为decimal(18,8)）
-- ============================================================
SELECT column_name,
       data_type,
       numeric_precision,
...
SELECT 'PK_CHECK' AS check_name,
       ...
       'prod_product_unit should have exactly 1 primary key' AS description
FROM pg_constraint c
JOIN pg_class t ON c.conrelid = t.oid
WHERE t.relname = 'prod_product_unit' AND c.contype = 'p';
...
FROM pg_indexes
WHERE tablename = 'prod_product_unit'
...
SELECT 'TABLE_COMMENT_CHECK' AS check_name,
       CASE WHEN obj_description('prod_product_unit'::regclass) IS NOT NULL THEN 'PASS' ELSE 'FAIL' END AS result;

SELECT c.column_name,
       col_description('prod_product_unit'::regclass, c.ordinal_position) AS comment,
       CASE WHEN col_description('prod_product_unit'::regclass, c.ordinal_position) IS NULL THEN 'MISSING' ELSE 'OK' END AS status
FROM information_schema.columns c
WHERE c.table_name = 'prod_product_unit'
...
SELECT column_name,
       is_nullable,
       CASE WHEN column_name IN ('id', 'tenant_id', 'product_id', 'unit_id', 'conversion_rate', 'created_at', 'updated_at', 'is_deleted', 'version')
             AND is_nullable = 'NO' THEN 'PASS'
            WHEN column_name IN ('id', 'tenant_id', 'product_id', 'unit_id', 'conversion_rate', 'created_at', 'updated_at', 'is_deleted', 'version')
             AND is_nullable = 'YES' THEN 'FAIL'
            ELSE 'OK' END AS result
FROM information_schema.columns
WHERE table_name = 'prod_product_unit'
  AND column_name IN ('id', 'tenant_id', 'product_id', 'unit_id', 'conversion_rate', 'created_at', 'updated_at', 'is_deleted', 'version');
...
WHERE script LIKE '%prod_product_unit%'
...
FROM pg_constraint c
JOIN pg_class t ON c.conrelid = t.oid
WHERE t.relname = 'prod_product_unit' AND c.contype = 'f';
...
FROM information_schema.columns
WHERE table_name = 'prod_product_unit' AND column_name = 'product_id';

SELECT 'COLUMN_UNIT_ID_CHECK' AS check_name,
       CASE WHEN count(*) > 0 THEN 'PASS' ELSE 'FAIL' END AS result,
       'unit_id column must exist (referenced by indexes)' AS description
FROM information_schema.columns
WHERE table_name = 'prod_product_unit' AND column_name = 'unit_id';

-- ============================================================
-- 16. BOOLEAN字段类型验证（is_base_unit）
-- ============================================================
SELECT column_name,
       data_type,
