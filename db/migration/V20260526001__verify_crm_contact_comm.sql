...
-- Description: crm_contact_comm 联系人通讯表DDL验证查询
-- Author: AI Generated
-- Date: 2026-06-01
-- Task: P0-003-005-004-001-003
-- ============================================================
...
WHERE schemaname = 'public'
  AND tablename = 'crm_contact_comm';
...
WHERE table_name = 'crm_contact_comm'
...
FROM pg_indexes
WHERE tablename = 'crm_contact_comm'
ORDER BY indexname;
...
WHERE rel.relname = 'crm_contact_comm'
...
FROM pg_class c
WHERE c.relname = 'crm_contact_comm'
...
FROM pg_attribute a
WHERE a.attrelid = 'crm_contact_comm'::regclass
  AND a.attnum > 0
...
WHERE script LIKE '%crm_contact_comm%'
...
BEGIN
    ...
    FROM pg_indexes
    WHERE tablename = 'crm_contact_comm'
      AND indexdef LIKE '%ON crm_contact_comm USING btree (tenant_id%';

    RAISE NOTICE '===== crm_contact_comm DDL验证结果汇总 =====';
    RAISE NOTICE '表存在: % (期望: 1)', v_table_count;
    RAISE NOTICE '字段总数: % (期望: 36)', v_col_count;
    RAISE NOTICE '索引总数: % (期望: 11)', v_idx_count;
    RAISE NOTICE '已注释字段: % (期望: 36)', v_cmt_count;
