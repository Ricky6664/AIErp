...
| P0-005-003-002-001-002 | 验证功能 | 2026-06-05T23:25 | ✅ | 列排序验证: 32/32测试通过; vue-tsc类型检查零错误(仅list-table组件); 逐项验证:列配置渲染/服务端排序(remote/multi/trigger)/列筛选(文本数字日期)/分页切换(via pageMode)/虚拟滚动(默认开启)/列宽localStorage持久化(viewCode前缀); vxe-table 4.19.4符合4.x要求; 浏览器性能测试(渲染/FPS/Network)需手动验证 | — |
