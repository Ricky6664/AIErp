# inv_stock 库存实时表 DDL 验证报告

> **任务编号**: P0-003-007-003-001-003
> **验证日期**: 2026-06-02
...
| 文件 | 版本号 | 说明 |
|------|--------|------|
| V20260526001__create_inv_stock.sql | V20260526001 | CREATE TABLE DDL |
| V20260526001__create_inv_stock_indexes.sql | V20260526001 | 索引与约束 DDL |
| V20260526001__create_inv_stock_rollback.sql | V20260526001 | 回滚脚本 |
| V20260526001__drop_inv_stock_indexes.sql | V20260526001 | 索引回滚脚本 |
...
| 检查项 | 结果 | 说明 |
|--------|:----:|------|
| 表名 inv_stock 符合命名规范 | ✅ PASS | 模块前缀 inv_ + 业务名 stock |
| 10 个通用字段完整 | ✅ PASS | id/tenant_id/created_at/updated_at/created_by/updated_by/is_deleted/owner_dept_id/owner_id/version 全部存在 |
| 字段总数 = 52 | ✅ PASS | 10 通用 + 20 业务 + 10 ext_str + 5 ext_num + 3 ext_date + 3 ext_bool + 1 ext_json |
...
| batch_no VARCHAR(50) 可空 | ✅ PASS | 批次号允许为空 |
...
| 检查项 | 结果 | 说明 |
|--------|:----:|------|
| qty 使用 DECIMAL(18,8) | ✅ PASS | 库存数量精度符合全局规范 |
| amount 使用 DECIMAL(18,8) | ✅ PASS | 库存金额精度符合全局规范 |
...
| 检查项 | 结果 | 说明 |
|--------|:----:|------|
| 主键约束 pk_inv_stock | ✅ PASS | ALTER TABLE RENAME CONSTRAINT 重命名 |
| 部分唯一索引 uk_inv_stock_order_no | ✅ PASS | WHERE is_deleted = false，避免"一删一活"陷阱 |
| tenant_id 联合索引首列 | ✅ PASS | idx_inv_stock_tenant_order_no/tenant_status/tenant_deleted/tenant_product_code/tenant_product_name/created_at 共 6 个均以 tenant_id 为首列 |
...
| product_code/product_name 索引 | ✅ PASS | 商品快照字段多租户查询索引 |
...
| 检查项 | 结果 | 说明 |
|--------|:----:|------|
| 表注释 | ✅ PASS | COMMENT ON TABLE inv_stock IS '库存实时表' |
| 10 个通用字段注释 | ✅ PASS | id/tenant_id/created_at/updated_at/created_by/updated_by/is_deleted/owner_dept_id/owner_id/version 均有注释 |
| 20 个业务字段注释 | ✅ PASS | product_id/warehouse_id/batch_no/qty/amount/order_no/order_date/status/remark 及商品快照字段均有注释 |
...
| 序号 | 严重级别 | 问题描述 | 所在文件 |
|:---:|:---:|---------|---------|
| 1 | ❌ MEDIUM | 22 个扩展字段缺少 COMMENT 注释（ext_str1-10/ext_num1-5/ext_date1-3/ext_bool1-3/ext_json） | V20260526001__create_inv_stock.sql |
| 2 | ⚠️ LOW | 所有 inv_stock 迁移脚本使用同一版本号 V20260526001，与项目其他 30+ 脚本版本冲突 | 项目全局 |
...
**总体结论**: DDL 结构设计正确，10 个通用字段完整，52 个字段数量精确，索引设计严谨（部分唯一索引含 WHERE is_deleted=false，6 个 tenant_id 为首列的联合索引覆盖多租户查询场景，product_id/warehouse_id/batch_no/status/order_date 均有独立索引），数值精度统一使用 DECIMAL(18,8)，无外键约束。存在 1 个需修复问题：22 个扩展字段缺少 COMMENT 注释。
