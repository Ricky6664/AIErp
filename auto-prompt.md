...
4. 自检通过后：
   ...
   e. **模块完成检查**：检查当前模块在 `tasks_active.md` 中的所有叶子任务是否全部 ✅
      ...
      - 未完成 → 跳过此步
   f. **🔴 铁律：必须跳到 Step 4 执行 `cmd.exe /c start` 命令启动接力，然后才能退出。不执行该命令 = 任务未完成。绝对禁止直接退出或继续认领下一个任务。**
5. 自检失败（连续 3 次修复同一问题仍不通过）：
   ...
   h. **🔴 跳到 Step 4 执行 `cmd.exe /c start` 命令启动接力，然后退出。禁止直接退出。**

---

## Step 4: 退出（铁律：必须先启动接力再退出）

1. 再读 `config.ini`
2. `single_stop=true` 或 `multi_stop=true` → 输出"停止开关已开启"→ 直接退出，不启动接力
3. 否则 → **用 Bash 工具执行以下命令（必须执行，不得跳过，不得修改）：**

```
cmd.exe /c start "ERP Worker" "D:\ClaudeCode\ErpProject\AiErp\auto.bat" W1
```

> 将 `W1` 替换为你的工人 ID。**这条命令必须执行成功后才能退出会话。不执行 = 任务未完成。**

**🚫 绝对禁止：**
- ❌ 回到 Step 2 认领下一个任务
- ❌ 不执行上面的命令就退出
- ❌ 说"auto.ps1 会处理"——不会，只有你执行这条命令才会启动下一个会话
