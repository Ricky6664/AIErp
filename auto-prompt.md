...
> **⚠️ 铁律：每次会话只执行 1 个叶子任务。完成 1 个任务后必须立即退出，绝对禁止继续认领下一个任务。** auto.ps1 的 `while($true)` 循环会在你退出后自动启动新的 Claude 会话来执行下一个任务。如果你在一个会话内执行了 2 个或更多任务，就是严重违规。
...
超时仍未获取 → 输出"获取锁失败"→ 退出（auto.ps1 循环会重试）
...
如果合格候选列表为空 → 释放锁 → 输出"当前无可认领任务，等待中"→ 退出（auto.ps1 循环会重试）
...
> **⚠️ 你已经完成了本次会话的唯一任务。必须立即退出，禁止执行任何后续操作。**
> auto.ps1 的 `while($true)` 循环会在你退出后自动启动新的 Claude 会话来执行下一个任务。

1. 再读 `config.ini`
2. `single_stop=true` 或 `multi_stop=true` → 输出"停止开关已开启"→ 退出
3. 否则 → 输出本次会话执行报告（完成了哪个任务）→ **立即退出**
...
3. 如果不存在 🔄 任务 → 项目已全部完成：
   ...
   → 直接退出（auto.ps1 循环会检测到无任务可执行并关闭窗口）
