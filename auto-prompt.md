...
> **⚠️ 铁律：每次会话只执行 1 个叶子任务。完成 1 个任务后必须立即跳到 Step 4（读 `.session-id` → 写 `.relay`），绝对禁止继续认领下一个任务。** 如果你在一个会话内执行了 2 个或更多任务，就是严重违规。
...
4. 自检通过后：
   ...
   e. **模块完成检查**：检查当前模块在 `tasks_active.md` 中的所有叶子任务是否全部 ✅
      ...
      - 未完成 → 跳过此步
   f. **🔴 铁律：必须跳到 Step 4 写接力标记（读 `.session-id` → 写 `.relay`），然后才能退出。不写标记 = 任务未完成。绝对禁止直接退出或继续认领下一个任务。**
5. 自检失败（连续 3 次修复同一问题仍不通过）：
   ...
   h. **🔴 跳到 Step 4 写接力标记（读 `.session-id` → 写 `.relay`），然后退出。禁止直接退出。**
...
3. 否则 → **执行以下两步（必须都执行，不得跳过）：**
   ...
   **a. 读取本次会话的 Session ID：**
   用 Read 工具读取文件 `{SCRIPT_DIR}\.session-id`，获取里面的 Session ID 字符串（形如 `W1-12345`）。

   **b. 用 Bash 工具写接力标记文件：**
   用 Bash 工具执行以下命令（将 YOUR_SESSION_ID 替换为你刚才读到的 Session ID）：

   ```bash
   echo|set /p="YOUR_SESSION_ID" > "{SCRIPT_DIR}\.relay"
   ```

   > **⚠️ 关键要求：`.relay` 文件的内容必须且只能是 Session ID 字符串，不能包含任何其他内容。**
   >
   > 例如 `.session-id` 的内容是 `W1-12345`：
   > - ✅ 正确：`.relay` 内容 = `W1-12345`（仅此 7 个字符）
   > - ❌ 错误：`.relay` 内容 = `session=W1-12345\nstatus=completed\n...`（多了其他内容）
   > - ❌ 错误：`.relay` 内容 = `next`（没有用 Session ID）
   >
   > auto.ps1 会**精确比对** `.relay` 的内容和 `.session-id` 的内容。不匹配 = 不会开新窗口。
   > **这两个步骤必须都完成后才能退出会话。**

**🚫 绝对禁止：**
- ❌ 回到 Step 2 认领下一个任务
- ❌ 不写 `.relay` 文件就退出
- ❌ 在 `.relay` 中写 Session ID 以外的任何内容（任务摘要、状态、下一个任务编号等）
- ❌ 在一个会话内执行多个任务
...
2. 如果存在 🔄 任务 → 输出"等待其他工人完成中"→ 直接退出（不写标记文件）
3. 如果不存在 🔄 任务 → 项目已全部完成：
