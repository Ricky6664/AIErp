...
[void]$wdLines.Add('')
[void]$wdLines.Add('    # Try to extract session ID using regex (handles any format)')
[void]$wdLines.Add('    $extractedId = ''''')
[void]$wdLines.Add('    if ($content -match ''W\d+-\d+'') { $extractedId = $Matches[0] }')
[void]$wdLines.Add('')
[void]$wdLines.Add('    if ($extractedId -eq $expectedSession) {')
[void]$wdLines.Add('        WLog "MATCH! Extracted session $extractedId. Waiting 10s..."')
[void]$wdLines.Add('    } elseif ($extractedId -ne '''') {')
[void]$wdLines.Add('        # Session ID in content but wrong one')
[void]$wdLines.Add('        $preview = $content.Substring(0, [Math]::Min(150, $content.Length)) -replace "`r`n", " " -replace "`n", " "')
[void]$wdLines.Add('        WLog "WRONG SESSION: expected $expectedSession but found $extractedId. Content: $preview. Deleting."')
[void]$wdLines.Add('        Remove-Item $relayFile -Force -ErrorAction SilentlyContinue')
[void]$wdLines.Add('        continue')
[void]$wdLines.Add('    } else {')
[void]$wdLines.Add('        # No session ID pattern found - content is malformed, but task IS done (.relay exists)')
[void]$wdLines.Add('        $preview = $content.Substring(0, [Math]::Min(150, $content.Length)) -replace "`r`n", " " -replace "`n", " "')
[void]$wdLines.Add('        WLog "NO SESSION ID in .relay, but spawning anyway. Content preview: $preview"')
[void]$wdLines.Add('    }')
